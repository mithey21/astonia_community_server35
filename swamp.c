    ...
    for (y = yf; y <= yt; y += 8) {
        for (x = xf; x <= xt; x += 8) {
            for (co = getfirst_char_sector(x, y); co; co = ch[co].sec_next) {
                if (abs(ch[co].x - xc) <= dist &&
                    abs(ch[co].y - yc) <= dist &&
                    (ch[co].flags & CF_PLAYER) &&
                    !(ch[co].flags & CF_INVISIBLE)) return 1;
            }
        }
    }
    return 0;
...
void swampspawn(int in, int cn) {
    ...
    if (!it[in].drdata[2] && co && (ch[co].flags) && ch[co].serial == ser) { // it's still alive
        if (!player_close(it[in].x, it[in].y, 8) && !player_close(ch[co].x, ch[co].y, 8)) remove_destroy_char(co); // remove swamp beast if no player is near anymore
        else *(unsigned int *)(it[in].drdata + 12) = ticker; // reset spawn timer each time we see our beast alive
        call_item(it[in].driver, in, 0, ticker + TICKS);
        return;
    }
    if (!it[in].drdata[2] && last && ticker - last < TICKS * 60 * 2) {
        call_item(it[in].driver, in, 0, ticker + TICKS);
        return;
    }
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    say(cn, "Greetings, %s! I am %s, First Sergeant of the Seyan'Du and commander of this outpost.", ch[co].name, ch[cn].name);
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 1:
                    if (ppd->kelly_state >= 15) ppd->clara_state++; // fall through...
                    else break;
                case 2:
                    say(cn, "I assume thou hast been sent from Aston, %s, to report on our status. The road through the swamp is no longer secure and we have been under attack from beasts emerging from the swamp.", get_army_rank_string(co));
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 3:
                    say(cn, "Under the current circumstances, I do not recommend sending reinforcements to secure the road. We cannot afford to bind our forces here. Now go back to Aston and deliver this report.");
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 4:
                    say(cn, "Afterwards come back here, I have more work for thee. That will be all, %s. Dismissed!", get_army_rank_string(co));
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 5:
                    if (ppd->kelly_state >= 18) ppd->clara_state++; // fall through...
                    else break; // waiting for player to reach kelly and come back
                case 6:
                    say(cn, "I have a difficult mission for thee, %s. The main reason we had to retreat to this camp was one huge swamp beast. It seemed to be immune to our attacks.", ch[co].name);
                    questlog_open(co, 21);
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 7:
                    ...
                    break;
                case 8:
                    say(cn, "Dismissed, %s. And good luck. Thou wilt need it.", get_army_rank_string(co));
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 9:
                    if ((in = ch[co].item[WN_RHAND]) && it[in].ID == IID_HARDKILL) {
                        if (questlog_count(co, 21) == 0) give_military_pts(cn, co, 4, EXP_AREA15_HARDKILL);
                        ppd->clara_state++;
                    } else break;
                case 10:
                    if ((in = ch[co].item[WN_RHAND]) && it[in].ID == IID_HARDKILL && it[in].drdata[37] < 36)
                    ...
                    break;
                case 11:
                    if ((in = ch[co].item[WN_RHAND]) && it[in].ID == IID_HARDKILL && it[in].drdata[37] >= 36) ppd->clara_state++;
                    else break;
                case 12:
                    say(cn, "Now that thou knowest how to kill that beast, please go and do it.");
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                case 13:
                    break; // waiting for kill
                case 14:
                    say(cn, "Well done indeed, %s!", ch[co].name);
                    questlog_done(co, 21);
                    ...
                    break;
                case 15:
                    say(cn, "The swamp will be safer now, but more dangers await thee on thy travels. May Ishtar be with thee, %s.", ch[co].name);
                    ppd->clara_state++;
                    didsay = 1;
                    break;
                }
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 2:
                ppd = set_data(co, DRD_AREA3_PPD, sizeof(struct area3_ppd));
                if (ppd && ppd->clara_state <= 5) {
                    dat->last_talk = 0;
                    ppd->clara_state = 0;
                }
                if (ppd && ppd->clara_state >= 6 && ppd->clara_state <= 9) {
                    dat->last_talk = 0;
                    ppd->clara_state = 6;
                }
                if (ppd && ppd->clara_state >= 10 && ppd->clara_state <= 11) {
                    dat->last_talk = 0;
                    ppd->clara_state = 10;
                }
                if (ppd && ppd->clara_state >= 12 && ppd->clara_state <= 13) {
                    dat->last_talk = 0;
                    ppd->clara_state = 12;
                }
                if (ppd && ppd->clara_state >= 15 && ppd->clara_state <= 16) {
                    dat->last_talk = 0;
                    ppd->clara_state = 15;
                }
                break;
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_SWAMPCLARA:
        clara_driver(cn, ret, lastact);
        return 1;
    case CDR_SWAMPMONSTER:
