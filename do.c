...
int do_walk(int cn, int dir) {
    ...
    if (ch[cn].flags & (CF_DEAD)) {
        error = ERR_DEAD;
        return 0;
    }

    if (!dx2offset(dir, &dx, &dy, &diag)) {
        error = ERR_ILLEGAL_DIR;
        return 0;
    }

    if (!char_can_flee(cn)) {
        error = ERR_CANNOT_ESCAPE;
        return 0;
    }
    ...
    if (map[m].flags & (MF_MOVEBLOCK | MF_TMOVEBLOCK)) {
        error = ERR_BLOCKED;
        return 0;
    }
    if (diag) {
        if (map[ch[cn].x + dx + (ch[cn].y) * MAXMAP].flags & (MF_MOVEBLOCK | MF_TMOVEBLOCK)) {
            error = ERR_BLOCKED;
            return 0;
        }
        if (map[ch[cn].x + (ch[cn].y + dy) * MAXMAP].flags & (MF_MOVEBLOCK | MF_TMOVEBLOCK)) {
            error = ERR_BLOCKED;
            return 0;
        }
    }
...
int do_take(int cn, int dir) {
    ...
    if (!dx2offset(dir, &x, &y, NULL)) {
        error = ERR_ILLEGAL_DIR;
        return 0;
    }

    if (ch[cn].citem) {
        error = ERR_HAVE_CITEM;
        return 0;
    }
...
int do_attack(int cn, int dir, int co) {
    ...
    if (co != map[m].ch &&
        ...
        error = ERR_NO_CHAR;
        return 0;
    } // the intended victim isnt reachable

    if (ch[co].flags & (CF_DEAD)) {
        error = ERR_DEAD;
        return 0;
    }
...
int do_give(int cn, int dir) {
    ...
    if ((ch[co].flags & CF_PLAYER) && ch[co].citem && cnt_free_inv(co) < 1) {
        error = ERR_BLOCKED;
        return 0;
    }
    if (!(ch[co].flags & CF_PLAYER) && ch[co].citem) {
        error = ERR_BLOCKED;
        return 0;
    }
...
int do_bless(int cn, int co) {
    ...
    if (warcried(cn)) {
        error = ERR_UNCONCIOUS;
        return 0;
    }

    if ((ch[cn].flags & CF_NOMAGIC) && !(ch[cn].flags & CF_NONOMAGIC)) {
        error = ERR_UNCONCIOUS;
        return 0;
    }
    ...
    if (!ch[cn].value[0][V_BLESS]) {
        error = ERR_UNKNOWN_SPELL;
        return 0;
    }

    if (ch[cn].mana < BLESSCOST) {
        error = ERR_MANA_LOW;
        return 0;
    }

    if ((ch[cn].flags & CF_PLAYER) && !(ch[co].flags & (CF_PLAYER | CF_PLAYERLIKE))) {
        error = ERR_NOT_PLAYER;
        return 0;
    }

    if (!add_same_spell(co, IDR_BLESS)) {
        error = ERR_ALREADY_WORKING;
        return 0;
    }
...
int do_freeze(int cn) {
    ...
    if (!ch[cn].value[0][V_FREEZE]) {
        error = ERR_UNKNOWN_SPELL;
        return 0;
    }

    if (ch[cn].mana < FREEZECOST) {
        error = ERR_MANA_LOW;
        return 0;
    }
...
int do_heal(int cn, int co) {
    ...
    if (ch[cn].mana < POWERSCALE) {
        error = ERR_MANA_LOW;
        return 0;
    } // no heals for less than one point
    ...
    if ((ch[cn].flags & CF_PLAYER) && areaID == 33) {
        error = ERR_UNKNOWN_SPELL;
        return 0;
    }
...
int do_fireball(int cn, int x, int y) {
    ...
    if (ch[cn].mana < FIREBALLCOST) {
        error = ERR_MANA_LOW;
        return 0;
    }

    if (!may_add_spell(cn, IDR_FIRE)) {
        error = ERR_ALREADY_WORKING;
        return 0;
    }
...
int do_earthrain(int cn, int x, int y, int strength) {
    ...
    if (ch[cn].hp - POWERSCALE < strength * 100) {
        error = ERR_MANA_LOW;
        return 0;
    }
...
int do_ball(int cn, int x, int y) {
    ...
    if (ch[cn].mana < FLASHCOST) {
        error = ERR_MANA_LOW;
        return 0;
    }

    if (!may_add_spell(cn, IDR_FLASHY)) {
        error = ERR_ALREADY_WORKING;
        return 0;
    }

    m = ch[cn].x + ch[cn].y * MAXMAP;
    if ((co = map[m + 1].ch) && can_attack(cn, co)) {
        error = ERR_ILLEGAL_COORDS;
        return 0;
    }
    if ((co = map[m - 1].ch) && can_attack(cn, co)) {
    ...
    if ((co = map[m + MAXMAP].ch) && can_attack(cn, co)) {
        error = ERR_ILLEGAL_COORDS;
        return 0;
    }
    if ((co = map[m - MAXMAP].ch) && can_attack(cn, co)) {
...
int do_magicshield(int cn) {
    ...
    if (ch[cn].mana < POWERSCALE) {
        error = ERR_MANA_LOW;
        return 0;
    } // no magic shield with less than one point
...
int do_warcry(int cn) {
    ...
    if (!ch[cn].value[0][V_WARCRY]) {
        error = ERR_UNKNOWN_SPELL;
        return 0;
    }

    if (ch[cn].endurance < WARCRYCOST) {
        error = ERR_MANA_LOW;
        return 0;
    }
...
int container(int cn, int pos, int flag, int fast) {
    ...
    if (pos < 0 || pos >= INVENTORYSIZE) {
        error = ERR_ILLEGAL_POS;
        return 0;
    }
    ...
    if (flag) { // swap
        ...
        if (noquest && in && (it[in].flags & IF_QUEST)) {
            error = ERR_QUESTITEM;
            return 0;
        }
...
int char_swap(int cn) {
    ...
    if (!(ch[co].flags & (CF_PLAYER | CF_PLAYERLIKE | CF_ALLOWSWAP))) {
        error = ERR_NOT_PLAYER;
        return 0;
    }
    ...
    if ((map[m].flags & MF_PEACE) && !(map[m2].flags & MF_PEACE)) {
        error = ERR_ACCESS_DENIED;
        return 0;
    }
    if (!(map[m].flags & MF_PEACE) && (map[m2].flags & MF_PEACE)) {
    ...
    if ((map[m].flags & MF_UNDERWATER) && !(map[m2].flags & MF_UNDERWATER)) {
        error = ERR_ACCESS_DENIED;
        return 0;
    }
    if (!(map[m].flags & MF_UNDERWATER) && (map[m2].flags & MF_UNDERWATER)) {
