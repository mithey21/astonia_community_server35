/*
 * $Id: club.c,v 1.2 2007/02/24 14:09:54 devel Exp $
 *
 * $Log: club.c,v $
...
void tick_club(void) {
    ...
    if (areaID == 3 && realtime - last_update > 30) {
        for (n = 0; n < MAXCLUB; n++) {
            ...
            if (club[n].money < 500 * 100) {
                xlog("club %d %s deleted", n, club[n].name);
                club[n].name[0] = 0;
            } else {
                club[n].money -= 500 * 100;
                club[n].paid += 60 * 60 * 24 * 7;
                xlog("club %d %s paid 500g, new money %d, new paid %d", n, club[n].name, club[n].money, club[n].paid);
            }
            db_update_club(n);
            break;
        }
...
void showclub(int cn) {
    ...
    if ((cnr = get_char_club(cn))) {
        ...
        if (rank < 0 || rank > 2) {
            elog("showclub(): got illegal clan rank (%s,%d,%d)", ch[cn].name, cn, rank);
            rank = 0;
        }

        log_char(cn, LOG_SYSTEM, 0, "You are a member of the club '%s' (%d), your rank is %s.", club[cnr].name, cnr, rankname[rank]);

        log_char(cn, LOG_SYSTEM, 0, "Your club has %d gold, the next payment of 500 gold is due in %d hours.",
                 club[cnr].money / 100,
