/*
 *
 * $Id: merchant.c,v 1.4 2008/03/27 08:22:44 devel Exp $
 *
 * $Log: merchant.c,v $
 * Revision 1.4  2008/03/27 08:22:44  devel
 * gender bender
 *
...
void merchant_driver(int cn, int ret, int lastact) {
    ...
    if (ticker > dat->last_talk + TICKS * 60 && !RANDOM(25)) {
        ...
        case 0:
            ...
            break;
        case 1:
            whisper(cn, "There's something stuck between thy teeth.");
            break;
        case 2:
        ...
        case 5:
            ...
            break;
        case 6:
            murmur(cn, "Ishtar! Oh, what has become of us!");
            break;
        case 7:
            murmur(cn, "The demons will get you.");
            break;
        case 8:
            emote(cn, "scratches %s back", hisname(cn));
            break;
        case 9:
            if (map[ch[cn].x + ch[cn].y * MAXMAP].flags & MF_INDOORS) emote(cn, "stares at the ceiling");
            else emote(cn, "stares at the sky");
            break;
        case 10:
            emote(cn, "twiddles %s thumbs", hisname(cn));
            break;
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_MERCHANT:
        merchant_driver(cn, ret, lastact);
        return 1;
    default:
        return 0;
    }
