...
void superior_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            if ((msg->dat1 == 1 || msg->dat1 == 2) && msg->dat3 != cn) { // talk, and not our talk
                if (strcasestr((char *)msg->dat2, "Kazimah") && dat->nr == 1) dat->stun = ticker + TICKS * 60;
                if (strcasestr((char *)msg->dat2, "Ergatoth") && dat->nr == 2) dat->stun = ticker + TICKS * 60;
                if (strcasestr((char *)msg->dat2, "Norganoth") && dat->nr == 3) dat->stun = ticker + TICKS * 60;
                if (strcasestr((char *)msg->dat2, "Larkanoth") && dat->nr == 4) dat->stun = ticker + TICKS * 60;
            }
        }
    ...
    if (ch[cn].hp < 10 * POWERSCALE) dat->mode = M_RUN;

    if (ch[cn].hp >= ch[cn].value[0][V_HP] * POWERSCALE && ch[cn].mana >= ch[cn].value[0][V_MANA] * POWERSCALE) dat->mode = M_FIGHT;
...
void shrine_driver(int in, int cn) {
    ...
    if (type == 0) {
        ...
        case 0:
        case 1:
            if (ch[cn].flags & CF_MAGE) in2 = create_item("mana_potion1");
            else in2 = create_item("healing_potion1");
            break;

        case 2:
        case 3:
            if (ch[cn].flags & CF_WARRIOR) in2 = create_item("healing_potion1");
            else in2 = create_item("mana_potion1");
            break;

        case 4:
        case 5:
            log_char(cn, LOG_SYSTEM, 0, "You have been blessed with experience.");
            give_exp(cn, 250);
            break;

        case 6:
            log_char(cn, LOG_SYSTEM, 0, "You have been protected for a short while.");
            add_bonus_spell(cn, IDR_ARMOR, 5 * 20, TICKS * 60 * 5);
            break;

        case 7:
            log_char(cn, LOG_SYSTEM, 0, "You are more dangerous for a short while.");
            add_bonus_spell(cn, IDR_WEAPON, 5 * 20, TICKS * 60 * 5);
            break;

        case 8:
            log_char(cn, LOG_SYSTEM, 0, "Your capacity was increased for a short while.");
            if (ch[cn].flags & CF_WARRIOR) add_bonus_spell(cn, IDR_HP, 15, TICKS * 60 * 5);
            else add_bonus_spell(cn, IDR_MANA, 15, TICKS * 60 * 5);
            break;

        case 9:
            log_char(cn, LOG_SYSTEM, 0, "Your capacity was increased for a short while.");
            if (ch[cn].flags & CF_MAGE) add_bonus_spell(cn, IDR_MANA, 15, TICKS * 60 * 5);
            else add_bonus_spell(cn, IDR_HP, 15, TICKS * 60 * 5);
            break;
        ...
        case 0:
        case 1:
            if (ch[cn].flags & CF_MAGE) in2 = create_item("mana_potion2");
            else in2 = create_item("healing_potion2");
            break;

        case 2:
        case 3:
            if (ch[cn].flags & CF_WARRIOR) in2 = create_item("healing_potion2");
        ...
        case 7:
            log_char(cn, LOG_SYSTEM, 0, "You have been protected for a while.");
            add_bonus_spell(cn, IDR_ARMOR, 10 * 20, TICKS * 60 * 15);
            break;

        case 8:
            log_char(cn, LOG_SYSTEM, 0, "You are more dangerous for a while.");
            add_bonus_spell(cn, IDR_WEAPON, 10 * 20, TICKS * 60 * 15);
            break;

        case 9:
            log_char(cn, LOG_SYSTEM, 0, "Your capacity was increased for a while.");
            if (ch[cn].flags & CF_WARRIOR) add_bonus_spell(cn, IDR_HP, 25, TICKS * 60 * 15);
            else add_bonus_spell(cn, IDR_MANA, 25, TICKS * 60 * 15);
            break;

        case 10:
            log_char(cn, LOG_SYSTEM, 0, "Your capacity was increased for a while.");
            if (ch[cn].flags & CF_MAGE) add_bonus_spell(cn, IDR_MANA, 25, TICKS * 60 * 15);
            else add_bonus_spell(cn, IDR_HP, 25, TICKS * 60 * 15);
            break;
        ...
        case 0:
        case 1:
            if (ch[cn].flags & CF_MAGE) in2 = create_item("mana_potion3");
        ...
        case 2:
        case 3:
            if (ch[cn].flags & CF_WARRIOR) in2 = create_item("healing_potion3");
...
void parkshrine_driver(int in, int cn) {
    ...
    case 1:
        new = !ppd->kelly_found1;
        ppd->kelly_found1 = 1;
        break;
    case 2:
        new = !ppd->kelly_found2;
        ppd->kelly_found2 = 1;
        break;
    case 3:
        new = !ppd->kelly_found3;
        ppd->kelly_found3 = 1;
        break;
    default:
        log_char(cn, LOG_SYSTEM, 0, "BUG #55343, please report");
        return;
    }
    if (new) {
...
void chestspawn_driver(int in, int cn) {
    ...
    if (cn && !it[in].drdata[1]) {
        ...
        switch (it[in].drdata[0]) {
        case 0:
            co = create_char("normal_vampire", 0);
            break;
        default:
            return;
        }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_SUPERIOR:
        superior_driver(cn, ret, lastact);
        return 1;
    case CDR_MOONIE:
        moonie_driver(cn, ret, lastact);
        return 1;
    case CDR_VAMPIRE:
        vampire_driver(cn, ret, lastact);
        return 1;
    case CDR_VAMPIRE2:
        vampire2_driver(cn, ret, lastact);
        return 1;
...
int it_driver(int nr, int in, int cn) {
    ...
    case IDR_SHRINE:
        ...
        return 1;
    case IDR_FIREBALL:
        fireball_machine(in, cn);
        return 1;
    case IDR_PARKSHRINE:
        parkshrine_driver(in, cn);
        return 1;
    case IDR_FLAMETHROW:
        flamethrow_driver(in, cn);
        return 1;
    case IDR_SPIKETRAP:
        spiketrap_driver(in, cn);
        return 1;
    case IDR_CHESTSPAWN:
        chestspawn_driver(in, cn);
        return 1;
    case IDR_EXTINGUISH:
        extinguish_driver(in, cn);
        return 1;
...
int ch_died_driver(int nr, int cn, int co) {
    ...
    case CDR_SUPERIOR:
        return 1;
    case CDR_VAMPIRE:
        vampire_dead_driver(cn, co);
        return 1;
    case CDR_MOONIE:
        return 1;
    case CDR_VAMPIRE2:
        vampire2_dead_driver(cn, co);
        return 1;
