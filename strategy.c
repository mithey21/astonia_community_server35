...
int driver(int type, int nr, int obj, int ret, int lastact) {
    ...
    case CDT_SPECIAL:
        return special_driver(nr, obj, (void *)(ret));
    default:
        return 0;
    }
}
...
void init_areas(void) {
    ...
    for (n = 1; n < MAXITEM; n++) {
        ...
        switch (it[n].driver) {
        case IDR_STR_SPAWNER:
            it[n].drdata[10] = area[slot].max_spawn; // set slot number for npc sprite calculation
            area[slot].spawn[area[slot].max_spawn++] = n;
        case IDR_STR_STORAGE:
        case IDR_STR_MINE:
        case IDR_STR_DEPOT:
            area[slot].item[area[slot].max_item++] = n;
            area[slot].used = 1;
            break;
        }
    }
    ...
    for (n = 0; n < MAX_STR_AREA; n++) {
        ...
        for (ai = pl = m = 0; m < area[n].max_spawn; m++) {
            ...
            if (*(unsigned int *)(it[in].drdata + 0) && *(unsigned int *)(it[in].drdata + 0) != 0xfffff000) {
                ...
                if (*(unsigned int *)(it[in].drdata + 0) < 0xfffff000) {
                    pl++;
                    winner = *(unsigned int *)(it[in].drdata + 0);
                } else ai++;
            }
        }
        if (pl == 1 && ai == 0) {
...
void setname(int cn, struct strategy_data *dat) {
    ...
    switch (dat->order) {
    case OR_MINE:
        sprintf(buf, "%s's Miner %d", dat->name, cn);
        break;
    case OR_FOLLOW:
        sprintf(buf, "%s's Minion %d", dat->name, cn);
        break;
    case OR_GUARD:
        sprintf(buf, "%s's Guard %d", dat->name, cn);
        break;
    case OR_ETERNALGUARD:
        sprintf(buf, "%s's E-Guard %d", dat->name, cn);
        break;
    case OR_FIGHTER:
    case OR_TAKE:
        sprintf(buf, "%s's Fighter %d", dat->name, cn);
        break;

    case OR_TRANSFER:
        sprintf(buf, "%s's Transfer %d", dat->name, cn);
        break;

    case OR_TRAIN:
        sprintf(buf, "%s's Trainee %d", dat->name, cn);
        break;

    default:
        sprintf(buf, "%s's Worker %d", dat->name, cn);
        break;
    }
...
void strategy_driver(int cn, int ret, int lastact) {
    ...
    case OR_TAKE:
        if (*(unsigned int *)(it[dat->or1].drdata + 0) == ch[cn].group) {
            if (dat->or2) {
                dat->order = OR_FIGHTER;
                dat->or1 = dat->or2;
                dat->or2 = 0;
    ...
    case OR_FOLLOW:
    case OR_FIGHTER:
        if (!ch[dat->or1].flags || ch[dat->or1].ID != ch[cn].group) {
            dat->order = dat->or1 = dat->or2 = 0;
            break;
        }
    ...
    case OR_GUARD:
    case OR_ETERNALGUARD:
        fight_driver_set_home(cn, dat->or1, dat->or2);
    ...
    default:
        if (!dat->or1) dat->or1 = findstorage(cn);
        if (abs(ch[cn].x - it[dat->or1].x) > 3 || abs(ch[cn].y - it[dat->or1].y) > 3) {
            if (move_driver(cn, it[dat->or1].x, it[dat->or1].y, 3)) return;
        }
        do_idle(cn, TICKS);
        ...
        if (msg->type == NT_CHAR) {
            ...
            if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) && ch[cn].driver != CDR_LOSTCON &&
                ...
                case 0:
                    ...
                    break;
                case 1:
                    if (get_army_rank_int(co) >= 8 && ch[co].level >= 30) ppd->boss_stage++;
                    else break;
                ...
                case 2:
                    ...
                    break;
                case 3:
                    say(cn, "Islena's Lieutenants are trying to gain control over the castles and the platinum mines. If they succeed, they could spawn a vast army of these artificial creatures, and overrun our newly established defenses around Aston.");
                    ppd->boss_stage++;
                    ppd->boss_timer = realtime;
                    break;
                case 4:
                    say(cn, "Unfortunately, we cannot spare any soldiers to fight the Lieutenants and their artificial armies. Therefore, we have to beat them using their own means.");
                    ppd->boss_stage++;
                    ppd->boss_timer = realtime;
                    break;
                case 5:
                ...
                case 8:
                    ...
                    break;
                case 9:
                    say(cn, "Good luck, %s. And report back from time to time.", ch[co].name);
                    ppd->boss_stage++;
                    ppd->boss_timer = realtime;
                    break;
                case 10:
                    ...
                    break;
                case 11:
                    if (ppd->boss_exp > ppd->boss_msg_exp) ppd->boss_stage = 10;
                    ppd->boss_timer = realtime;
                    break;
                }
            }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_STRATEGY:
        strategy_driver(cn, ret, lastact);
        return 1;
    case CDR_STRATEGY_BOSS:
        strategy_boss(cn, ret, lastact);
        return 1;
...
void task_mine(int n) {
    ...
    t = ad->an[n].target;
    update_npc_place(n);
    if (t != ad->an[n].current && ad->ap[t].parent != ad->an[n].current) {
        subtask_move(n);
        return;
    }
...
void ai_main(int in, unsigned int code) {
    ...
    if (ad->panic) {
        ...
        if (!missing && ad->free_workers > 0 && ticker - ad->lastchange > TICKS * 20) {
            ad->worklevel = min(MAXWORKER, ad->worklevel + 1);
            ad->lastchange = ticker;
        }
        if (missing && ad->free_workers == 0 && ticker - ad->lastchange > TICKS * 10) {
            ad->worklevel = max(1, ad->worklevel - 1);
            ad->lastchange = ticker;
        }
...
int str_exp_cost(struct strategy_ppd *ppd, int nr) {
    switch (nr) {
    case 1:
        if (ppd->income < 20) return 25;
        else return 0;
    case 2:
        if (ppd->max_level < 115) return 4;
        else return 0;
    case 3:
        if (ppd->max_worker < 16) return 10;
        else return 0;
    case 4:
        if (ppd->trainspeed < 8) return 4;
        else return 0;
    case 5:
        if (ppd->warcry < 115) return 4;
        else return 0;
    case 6:
        if (ppd->endurance < 115) return 4;
        else return 0;
    case 7:
        if (ppd->speed < 115) return 6;
        else return 0;
    case 8:
        if (ppd->eguardlvl < 115) return 3;
        else return 0;
...
int str_increment(struct strategy_ppd *ppd, int nr) {
    switch (nr) {
    case 1:
        if (str_exp_cost(ppd, nr)) return 1;
        else return 0;
    case 2:
        if (str_exp_cost(ppd, nr)) return 2;
        else return 0;
    case 3:
        if (str_exp_cost(ppd, nr)) return 1;
        else return 0;
    ...
    case 5:
        if (str_exp_cost(ppd, nr)) return 5;
        else return 0;
    case 6:
        if (str_exp_cost(ppd, nr)) return 5;
        else return 0;
...
int str_raise(int cn, struct strategy_ppd *ppd, int nr) {
    ...
    switch (nr) {
    case 1:
        ppd->income = min(ppd->income + str_increment(ppd, nr), 20);
        break;
    case 2:
        ppd->max_level = min(ppd->max_level + str_increment(ppd, nr), 115);
        break;
    case 3:
        ppd->max_worker = min(ppd->max_worker + str_increment(ppd, nr), 24);
        break;
    case 4:
        ppd->trainspeed = min(ppd->trainspeed + str_increment(ppd, nr), 8);
        break;
    case 5:
        ppd->warcry = min(ppd->warcry + str_increment(ppd, nr), 115);
        break;
    case 6:
        ppd->endurance = min(ppd->endurance + str_increment(ppd, nr), 115);
        break;
    case 7:
        ppd->speed = min(ppd->speed + str_increment(ppd, nr), 115);
        break;
    case 8:
        ppd->eguardlvl = min(ppd->eguardlvl + str_increment(ppd, nr), 115);
        break;
    default:
        log_char(cn, LOG_SYSTEM, 0, "Please report bug #4371g");
        return 0;
    }
    ppd->exp -= cost;
