...
static void solve_pents(int cc) {
    ...
    for (cn = getfirst_char(); cn; cn = getnext_char(cn)) {
        ...
        for (n = exp = 0; n < 6; n++) {
            exp += nppd->pent_worth[n];
            nppd->pent_value[n] = nppd->pent_worth[n] = 0;
...
int it_driver(int nr, int in, int cn) {
    ...
    case IDR_PENT:
        pent_driver(in, cn);
        return 1;
    case IDR_PENTBOSSDOOR:
        pentboss_door_driver(in, cn);
        return 1;
