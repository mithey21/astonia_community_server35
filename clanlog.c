...
void clanlog_help(int cn) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "-s <hours>: restrict output to entries not more than hours old");
...
int cmd_clanlog(int cn, char *ptr) {
    ...
    while (*ptr) {
        ...
        switch (*ptr) {
        case 'p':
            ptr = clanlog_player(cn, ptr + 1, &coID, &repeat);
            if (!ptr) return 1;
            if (repeat) return 0;
            break;
        case 'c':
            ptr = clanlog_clan(cn, ptr + 1, &clan, &serial);
            if (!ptr) return 1;
            break;
        case 'x':
            ptr = clanlog_prio(cn, ptr + 1, &prio);
            if (!ptr) return 1;
            break;
        case 's':
            ptr = clanlog_hours(cn, ptr + 1, &start);
            if (!ptr) return 1;
            break;
        case 'e':
            ptr = clanlog_hours(cn, ptr + 1, &end);
            if (!ptr) return 1;
            break;
        case 'i':
            ptr++;
