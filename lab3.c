...
void lab3_passguard_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            // is he at home
            if (ch[cn].x != ch[cn].tmpx || ch[cn].y != ch[cn].tmpy) {
                remove_message(cn, msg);
                continue;
            }
            ...
            // get lab ppd
            if (!(ppd = set_data(co, DRD_LAB_PPD, sizeof(struct lab_ppd)))) {
                remove_message(cn, msg);
                continue;
            }
            ...
            if (ticker < dat->last_talk + 5 * TICKS) {
                remove_message(cn, msg);
                continue;
            }
            ...
            case 0:
                say(cn, "Halt, %s, say the password, or leave this place immediately!", ch[co].name);
                didsay = 1;
                ppd->guard_talkstep++;
                break;
            case 1:
            ...
            case 2:
                ...
                break;
            case 3:
                say(cn, "I'll count up to three, then I will kill thee, %s. So move, or say the password!", ch[co].name);
                didsay = 1;
                ppd->guard_talkstep++;
                break;
            case 4:
            ...
            case 5:
                ...
                break;
            case 6:
                say(cn, "Three! %s, I'm coming!", ch[co].name);
                ppd->guard_talkstep = 0;
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (strcasestr(str, password)) {
                say(cn, "Fine, %s, I will open the door for thee. Mayest thou pass the last gate.", ch[co].name);
                ppd->guard_talkstep = 20;
            } else if (strcasestr(str, "BLUB")) {
                say(cn, "What?");
                dat->last_talk = ticker + 10 * TICKS;
...
void lab3_prisoner_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (strcasestr(str, "BLUB")) {
                say(cn, "Blub!");
                ppd->prisoner_talkstep = 0;
                dat->next_talk = ticker + TICKS * 5;
            } else if (strcasestr(str, "REPEAT")) {
                say(cn, "Blub.");
                ppd->prisoner_talkstep = 0;
                dat->next_talk = ticker + TICKS * 1;
            }
        }
...
void lab3_plant(int in, int cn) {
    ...
    if (drdata[0] == 1 || drdata[0] == 2 || drdata[0] == 3) {
        ...
        if (drdata[0] == 1) {
            if (drdata[1] / 2 == 3) in2 = create_item("lab3_yellowberry_three");
                ...
                log_char(cn, LOG_SYSTEM, 0, "At the moment this plant has no berries to pick!");
                return;
            }
    ...
    if (drdata[0] == 5 || drdata[0] == 6) {
        ...
        if (drdata[0] == 5) { // eating yellow berries
            ...
            if (!add_spell(cn, IDR_OXYGEN, TICKS * val, "nonomagic_spell")) {
                log_char(cn, LOG_SYSTEM, 0, "Due to some strange reasons thou canst not eat %s now.", drdata[1] == 1 ? "that berry" : "those berries");
                return;
            }
    ...
    if (drdata[0] == 11) {
        // create effect
        if (!add_spell(cn, IDR_UWTALK, 10 * TICKS, "nonomagic_spell")) {
            log_char(cn, LOG_SYSTEM, 0, "Thou art still chewing a brown berry.");
            return;
        }
...
void lab3_special(int in, int cn) {
    ...
    if (drdata[0] == 1) {
        ...
        if (map[ch[cn].x + ch[cn].y * MAXMAP].flags & MF_UNDERWATER) {
            ...
            for (slot = 30; slot < INVENTORYSIZE; slot++)
                if ((in2 = ch[cn].item[slot]) && it[in2].driver == IDR_TORCH && it[in2].drdata[0]) {
                    item_driver(IDR_TORCH, in2, cn);
                    extinguished++;
                }
            ...
            if ((in2 = ch[cn].item[WN_LHAND]) && it[in2].driver == IDR_TORCH && it[in2].drdata[0]) {
                item_driver(IDR_TORCH, in2, cn);
                extinguished++;
            }
    ...
    if (drdata[0] == 3) {
        ...
        case 20:
            if (!(ppd = set_data(cn, DRD_LAB_PPD, sizeof(struct lab_ppd)))) break;
        ...
        default:
            xlog("Invalid note at %s %d", __FILE__, __LINE__);
            break;
        }
...
int ch_died_driver(int nr, int cn, int co) {
    switch (nr) {
    case CDR_LAB3PASSGUARD:
        return 1;
    case CDR_LAB3PRISONER:
        return 1;
    default:
        return 0;
    }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_LAB3PASSGUARD:
        lab3_passguard_driver(cn, ret, lastact);
        return 1;
    case CDR_LAB3PRISONER:
        lab3_prisoner_driver(cn, ret, lastact);
        return 1;
    default:
        return 0;
    }
