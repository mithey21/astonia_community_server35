/*
 * $Id: create.c,v 1.20 2007/09/21 10:59:55 devel Exp $
 *
 * $Log: create.c,v $
 * Revision 1.20  2007/09/21 10:59:55  devel
 * tactics fix
 *
 * Revision 1.19  2007/08/15 09:13:23  devel
 * removed some log outputs
 *
 * Revision 1.18  2007/07/28 18:50:50  devel
 * basic account gets only first modifier on each item
 *
 * Revision 1.17  2007/07/09 11:21:03  devel
 * CF_xxx changes
 *
 * Revision 1.16  2007/07/04 09:20:25  devel
 * profession changes
 *
 * Revision 1.15  2007/06/28 12:11:49  devel
 * first kill error message less sensitive
 *
 * Revision 1.14  2007/06/22 13:02:19  devel
 * fixed bug in CF_xxx list
 *
 * Revision 1.13  2007/06/13 09:27:20  devel
 * removed some unwanted error messages
 *
 * Revision 1.12  2007/06/11 10:05:10  devel
 * removed IF_BEYOND...
 * added hpboost, manaboost
 * added V_COLD to the list og ignored stuff
 *
 * Revision 1.11  2007/06/08 12:08:05  devel
 * added/fixed some sanity checks
 *
 * Revision 1.10  2007/06/07 15:03:15  devel
 * added check for tactics and magic shield
 * fixed bug in check for immortal and first kill
 *
 * Revision 1.9  2007/05/03 14:33:42  devel
 * made speed loss of warcry and freeze not stack
 *
 * Revision 1.8  2007/04/12 12:00:40  devel
 * fixed bug in lifeshield calc
 *
 * Revision 1.7  2007/04/12 11:51:53  devel
 * update_char now reduces lifeshield to legal values
 *
 * Revision 1.6  2007/02/28 12:43:44  devel
 * added a log message
 *
...
static char *load_zone(char *name) {
    ...
    if (handle == -1) {
        elog("ERROR: Could not open zone \"%s\".", name);
        return NULL;
    }
    ...
    if (len == -1) {
        close(handle);
        elog("ERROR: Could not seek to end of zone \"%s\".", name);
        return NULL;
    }

    if (lseek(handle, 0, SEEK_SET) == -1) {
        close(handle);
        elog("ERROR: Could not seek to start of zone \"%s\".", name);
        return NULL;
    }
    ...
    if (!text) {
        close(handle);
        elog("ERROR: Could not malloc memory for zone \"%s\".", name);
        return NULL;
    }

    if (read(handle, text, len) != len) {
        xfree(text);
        close(handle);
        elog("ERROR: Could not read zone \"%s\".", name);
        return NULL;
    }
    text[len] = 0;
...
static int process_item(char *ptr) {
    ...
    while ((ptr = get_text(name, value, ptr))) {
        ...
        if (value[0] == ';') {
            if (!add_item(&itt)) {
                elog("Error in line %d: add_item() failed.", line);
                return 0;
            }
            gotname = 0;
            continue;
        ...
        // parse values
        if (!strcasecmp(name, "name")) safecpy(itt.item.name, value) else if (!strcasecmp(name, "description")) safecpy(itt.item.description, value) else if (!strcasecmp(name, "quality")) itt.item.quality = atoi(value);
        else if (!strcasecmp(name, "complexity")) itt.item.complexity = atoi(value);
            ...
            tmp = lookup_V(value);
            if (tmp == -1) {
                elog("Error in line %d: Unknown V_name \"%s\".", line, value);
                return 0;
            }
            ...
            if (cmod >= MAXMOD) {
                elog("Error in line %d: MAXMOD (%d) exceeded.", line, MAXMOD);
                return 0;
            }
            ...
            tmp = lookup_IF(value);
            if (tmp == -1) {
                elog("Error in line %d: Unknown IF \"%s\".", line, value);
                return 0;
            }
            itt.item.flags |= (1ull << tmp);
        } else {
            elog("Error in line %d: unknown name \"%s\".", line, name);
            return 0;
        }
    }
    if (gotname) {
        elog("Error in line %d: Premature end of input (or syntax error).", line);
        return 0;
    }
    return 1;
}
...
static int add_char(struct ch_temp *cht) {
    ...
    if ((cht->ch.flags & CF_QUESTITEM) &&
        ...
        strcmp(cht->ch.name, "Guild Master") &&
        strcmp(cht->ch.name, "Scarcewind"))
        elog("char %s has questitem but no immortal/noattack flag", cht->ch.name);
    if (!cht->balanced &&
        !(cht->ch.flags & CF_IMMORTAL) &&
        areaID != 1 &&
        strcmp(cht->ch.name, "Newbie") &&
        strcmp(cht->ch.name, "Dark Skeleton") &&
        strcmp(cht->ch.name, "Hrus-tak-lan") &&
        strcmp(cht->ch.name, "Tunnel Creeper") &&
        strcmp(cht->ch.name, "Clanvault"))
        elog("char %s is not immortal and not balanced", cht->ch.name);
...
static int process_char(char *ptr) {
    ...
    while ((ptr = get_text(name, value, ptr))) {
        ...
        // error checking...
        if (!gotname && value[0] != ':') {
            elog("Error in line %d: Need name before values.", line);
            return 0;
        }
        if (gotname && value[0] == ':') {
            elog("Error in line %d: Need semicolon first.", line);
            return 0;
        }
        if (!gotname && value[0] == ';') {
            elog("Error in line %d: Need name before semicolon.", line);
            return 0;
        }
        ...
        if (value[0] == ';') {
            if (!add_char(&cht)) {
                elog("Error in line %d: add_char() failed.", line);
                return 0;
            }
            gotname = 0;
            citem = 30;
        ...
        // parse values
        if (!strcasecmp(name, "name")) safecpy(cht.ch.name, value) else if (!strcasecmp(name, "description")) safecpy(cht.ch.description, value) else if (!strcasecmp(name, "sprite")) cht.ch.sprite = atoi(value);
        ...
        else if (!strcasecmp(name, "balanced")) cht.balanced = atoi(value);
        else if (!strcasecmp(name, "hpboost")) cht.hpboost = atoi(value);
        else if (!strcasecmp(name, "manaboost")) cht.manaboost = atoi(value);
        else if (!strcasecmp(name, "arg")) {
            if (cht.ch.arg) {
                cht.ch.arg = xrealloc(cht.ch.arg, strlen(cht.ch.arg) + strlen(value) + 1, IM_CHARARGS);
                if (!cht.ch.arg) {
                    elog("Error in line %d: Memory low!", line);
                    return 0;
                }
            ...
            if (tmp == 0) {
                elog("Error in line %d: Unknown item name \"%s\".", line, value);
                return 0;
            }
            if (citem >= INVENTORYSIZE) {
                elog("Error in line %d: Carrying capacity (%d) exceeded.\n", line, INVENTORYSIZE - 30);
                return 0;
            }
            ...
            tmp = lookup_item(value);
            if (tmp == 0) {
                elog("Error in line %d: Unknown spell name \"%s\".", line, value);
                return 0;
            }
            if (cspell >= 30) {
                elog("Error in line %d: Carrying capacity (18) exceeded.\n", line);
                return 0;
            }
            ...
            tmp = lookup_item(value);
            if (tmp == 0) {
                elog("Error in line %d: Unknown r-item name \"%s\".", line, value);
                return 0;
            }
            if (ritem >= MAXRANDITEM) {
                elog("Error in line %d: Random carrying capacity (%d) exceeded.\n", line, MAXRANDITEM);
                return 0;
            }
            if (!cht.rand_prob[ritem]) {
                elog("Error in line %d: No random value set.\n", line);
                return 0;
            }
            ...
            tmp = atoi(value);
            if (tmp < 1 || tmp > 9999) {
                elog("Error in line %d: Illegal probability \"%s\".", line, value);
                return 0;
            }
            ...
            tmp = lookup_CF(value);
            if (tmp == -1) {
                elog("Error in line %d: Unknown CF \"%s\".", line, value);
                return 0;
            }
...
int create_item_nr(int tmp) {
    ...
    if (it[n].ID == IID_ALCHEMY_INGREDIENT) {
        if (it[n].drdata[0] == 0x17 && !it[n].complexity) {
            ...
            it[n].complexity = 23 + die(2, 25);
        }
        if (it[n].drdata[0] == 0x15 && !it[n].complexity) {
            it[n].quality = 36 + die(2, 38);
            it[n].complexity = 36 + die(2, 38);
        }
        if (it[n].drdata[0] == 0x16 && !it[n].complexity) {
            it[n].quality = 48 + die(2, 50);
            it[n].complexity = 48 + die(2, 50);
        }
        if (it[n].drdata[0] == 0x18 && !it[n].complexity) {
            it[n].quality = 60 + die(2, 62);
            it[n].complexity = 60 + die(2, 62);
        }
    }

    if (it[n].flags & IF_WEAR) {
        str = get_complexity_cost(n);
        if (str != it[n].complexity) {
            elog("%s (%s) needs to have complexity %d, but has %d. fixed.", it[n].name, it[n].description, str, it[n].complexity);
            it[n].complexity = str;
        }
        if (str && it[n].quality > str * 1.2001) {
            elog("%s (%s) has a quality of %d but %d would be 120%% already. fixed.", it[n].name, it[n].description, it[n].quality, (int)(str * 1.20));
            it[n].quality = (int)(str * 1.2001);
        }
        if (str && it[n].quality < str * 0.9) {
            elog("%s (%s) has a quality of %d but that would be less than 90%%. fixed.", it[n].name, it[n].description, it[n].quality);
            it[n].quality = (int)(str * 0.9);
        }
        if (str && !(it[n].flags & IF_NOENHANCE)) it[n].value = it[n].quality * 10;
    }
...
int balance_tab(int level) {
    // tweak here to change the balance of the game:
    // this is the modifier that all balanced NPCs get
    // added to their stats!
    return (int)(level / 3.333333333);
}
...
int create_char_nr(int ctmp, int tmpa) {
    ...
    ch[n].value[1][V_HP] += ch_temp[ctmp].hpboost;
    ch[n].value[1][V_MANA] += ch_temp[ctmp].manaboost;
    ...
    if (ch[n].value[1][V_TACTICS] && ch[n].value[1][V_BLESS]) elog("%s (lvl %d) has both, Tactics and Bless!", ch[n].name, ch[n].level);
    if (ch[n].value[1][V_TACTICS] && ch[n].value[1][V_MAGICSHIELD]) elog("%s (lvl %d) has both, Tactics and Magic Shield!", ch[n].name, ch[n].level);
    if (!(ch[n].flags & CF_IMMORTAL) &&
        !ch[n].class &&
        !(ch[n].flags & CF_PLAYERLIKE) &&
        !(ch[n].flags & CF_NOPLRATT) &&
        strcmp(ch[n].name, "Gatekeeper") &&
    ...
    if (ch_temp[ctmp].special_prob && RANDOM(ch_temp[ctmp].special_prob) == 0) {
        in = create_special_item(ch_temp[ctmp].special_str, 5, 10000);
        if (in) {
            for (f = 30; f < INVENTORYSIZE && ch[n].item[f]; f++);
            if (f < INVENTORYSIZE) {
                ch[n].item[f] = in;
    ...
    if (ch_temp[ctmp].balanced && balance_tab(ch[n].level) > 0) {
        for (m = 12, f = 0; m < 30; m++) {
            if (!(ch[n].item[m])) {
                switch (f) {
                case 0:
                    in = create_item("base_balance");
                    break;
                case 1:
                    in = create_item("warrior_balance");
                    break;
                case 2:
                    in = create_item("mage_balance");
                    break;
                case 3:
                    in = create_item("misc_balance");
                    break;
                default:
                    in = 0;
                }
                if (it[in].mod_index[0] > 0) it[in].mod_value[0] = balance_tab(ch[n].level);
                if (it[in].mod_index[1] > 0) it[in].mod_value[1] = balance_tab(ch[n].level);
                if (it[in].mod_index[2] > 0) it[in].mod_value[2] = balance_tab(ch[n].level);
                if (it[in].mod_index[3] > 0) it[in].mod_value[3] = balance_tab(ch[n].level);
                if (it[in].mod_index[4] > 0) it[in].mod_value[4] = balance_tab(ch[n].level);
...
int alloc_char(void) {
    ...
    tmp = fchar;
    if (!tmp) {
        elog("alloc_char: MAXCHARS reached!");
        return 0;
    }
    fchar = tmp->next;
    cn = tmp - ch;
...
void update_char(int cn) {
    ...
    for (n = 0; n < 30; n++) {
        if ((in = ch[cn].item[n])) {
            if (it[in].complexity > 0) {
                if ((int)(it[in].complexity * 1.2) > it[in].quality) effectivity = (double)it[in].quality / it[in].complexity;
                else effectivity = 1.2;
            ...
            for (m = cnt = 0; m < MAXMOD; m++) {
                ...
                if (v1 >= 0) { // negative indizes are requirements
                    ...
                    else if (v1 == V_SPEED && it[in].driver == IDR_WARCRY) spellspeed = min(spellspeed, it[in].mod_value[m]);
                    else if (v1 == V_SPEED && it[in].driver == IDR_FREEZE) spellspeed = min(spellspeed, it[in].mod_value[m]);
                    else mod[v1] += v;
                }
            }
        }
    }
    mod[V_SPEED] += spellspeed;
    ...
    for (n = 0; n < V_MAX; n++) {
        ...
        // Set total value
        if (!ch[cn].value[1][n] && n >= V_DAGGER) ch[cn].value[0][n] = 0;
        else {
            if (n <= V_STR || n >= V_DAGGER) {
                mod[n] = min((int)(ch[cn].value[1][n] * 0.500), mod[n]); // reduce item and spell mods to max 50% of base value
                bless[n] = min((int)(ch[cn].value[1][n] * 0.500), bless[n]); // reduce bless mods to 50%
            }
    ...
    if ((ch[cn].flags & CF_PLAYER) && (!(ch[cn].flags & (CF_GOD)) || (ch[cn].sprite >= 60 && ch[cn].sprite < 120) || ch[cn].sprite == 27 || ch[cn].sprite == 157 || ch[cn].sprite == 39)) {
        ...
        case (CF_MAGE | CF_MALE):
            sbase = 60;
            break;
        case (CF_MAGE | CF_MALE | CF_ARCH):
            sbase = 65;
            break;
        case (CF_MAGE | CF_FEMALE):
            sbase = 75;
            break;
        case (CF_MAGE | CF_FEMALE | CF_ARCH):
            sbase = 70;
            break;

        case (CF_WARRIOR | CF_MALE):
            sbase = 85;
            break;
        case (CF_WARRIOR | CF_MALE | CF_ARCH):
            sbase = 80;
            break;
        case (CF_WARRIOR | CF_FEMALE):
            sbase = 95;
            break;
        case (CF_WARRIOR | CF_FEMALE | CF_ARCH):
            sbase = 90;
            break;

        case (CF_MAGE | CF_WARRIOR | CF_MALE):
            sbase = 105;
            break;
        case (CF_MAGE | CF_WARRIOR | CF_MALE | CF_ARCH):
            sbase = 100;
            break;
        case (CF_MAGE | CF_WARRIOR | CF_FEMALE):
            sbase = 115;
            break;
        case (CF_MAGE | CF_WARRIOR | CF_FEMALE | CF_ARCH):
            sbase = 110;
            break;
    ...
    ch[cn].value[0][V_OFFENSE] = get_attack_skill(cn) + mod[V_OFFENSE];
    ch[cn].value[0][V_DEFENSE] = get_parry_skill(cn) + mod[V_DEFENSE];
    ch[cn].value[0][V_WEAPON] += ch[cn].value[0][V_STR] * 20 / 8; //(1 ... 25)

    if (ch[cn].lifeshield > ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE / MAGICSHIELDMOD)
