...
static int enter_room(int cn, int class, int xs, int ys) {
    ...
    switch (class) {
    case 5:
        co = create_char("gatekeeper_w", 0);
        break;
    case 6:
        co = create_char("gatekeeper_m", 0);
        break;
    case 7:
        co = create_char("gatekeeper_s", 0);
        break;
    case 8:
        co = create_char("gatekeeper_s", 0);
        break;
...
static int enter_test(int cn, int class) {
    ...
    dlog(cn, 0, "entered gatekeeper room");
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    say(cn, "Be greeted, %s. These are the halls of Ishtar. Only the greatest fighters and magic users come here, to take the final test and fight the Gatekeeper.", ch[co].name);
                    ppd->welcome_state++;
                    didsay = 1;
                    break;
                case 1:
                    say(cn, "Those who succeed in this test will be able to enhance their abilities further. They may either choose to learn more about their profession than any other mortal being, or to start again as one who can learn all arts.");
                    ppd->welcome_state++;
                    didsay = 1;
                    break;
                case 2:
                    if (teleport_next_lab(co, 0)) {
                ...
                case 3:
                    if (!teleport_next_lab(co, 0)) ppd->welcome_state++; // fall through intended
                    else break;
                ...
                case 4:
                    ...
                    break;
                case 5:
                    say(cn, "Name the class thou wishest to become to begin the test. Each try will cost thee 100 gold coins.");
                    ppd->welcome_state++;
                    didsay = 1;
                    break;
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 7:
            case 8:
                if (!enter_test(co, didsay)) {
                    say(cn, "That is not a possible choice.");
                }
                break;
            case 9:
                if (ch[co].flags & CF_GOD) del_data(co, DRD_LAB_PPD);
                break;
            }
            if (didsay) {
                dat->last_talk = ticker;
...
void gate_fight_driver(int cn, int ret, int lastact) {
    ...
    // re-bless every 60 seconds to avoid some cheats
    slot = add_same_spell(cn, IDR_BLESS);
    if ((in = ch[cn].item[slot])) {
        t = *(signed long *)(it[in].drdata + 4);
        if (ticker - t > 60 * 24) {
            if (do_bless(cn, cn)) return;
        }
    }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_GATE_WELCOME:
        gate_welcome_driver(cn, ret, lastact);
        return 1;
    case CDR_GATE_FIGHT:
        gate_fight_driver(cn, ret, lastact);
        return 1;
