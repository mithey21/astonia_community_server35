/*
 * $Id: player.c,v 1.26 2008/04/21 09:09:32 devel Exp $ (c) 2005 D.Brockhaus
 *
 * $Log: player.c,v $
 * Revision 1.26  2008/04/21 09:09:32  devel
 * fixed lag message
 *
 * Revision 1.25  2008/03/29 15:46:36  devel
 * use command no longer able to access the lower half of the inventory for unpaid accounts
 *
 * Revision 1.24  2007/12/03 10:08:37  devel
 * username -> charactername
 *
 * Revision 1.23  2007/08/21 22:05:34  devel
 * *** empty log message ***
 *
 * Revision 1.22  2007/07/28 18:54:54  devel
 * display basic account unusable inventory slots as x
 *
 * Revision 1.21  2007/07/24 18:56:30  devel
 * changed fix-account instructions for new website
 *
 * Revision 1.20  2007/07/13 15:47:16  devel
 * clog -> charlog
 *
 * Revision 1.19  2007/07/11 12:38:16  devel
 * fixed one-pixel-off bug in MS
 *
 * Revision 1.18  2007/07/09 11:23:59  devel
 * added check for depot closing
 *
 * Revision 1.17  2007/07/04 09:23:06  devel
 * removed old professions
 *
 * Revision 1.16  2007/06/28 12:12:33  devel
 * added display of punishments on login
 *
 * Revision 1.15  2007/06/13 15:24:36  devel
 * added logout reasons for new punishment system
 *
 * Revision 1.14  2007/06/07 15:28:54  devel
 * removed PK
 *
 * Revision 1.13  2007/06/07 15:04:22  devel
 * changed max junk price to 25g due to reduced item prices
 *
 * Revision 1.12  2007/05/25 12:25:26  devel
 * added server is full message to login
 *
...
void kick_char(int cn) {
    ...
    ch[cn].logout_time = realtime;

    if (!save_char(cn, ch[cn].tmpa)) elog("could not save char %s %d!!", ch[cn].name, ch[cn].ID);
...
static void read_login(int nr) {
    ...
    prof = prof_start(29);
    ret = find_login(name, password, &area, &cn, &mirror, &ID, htonl(player[nr]->addr), &timeleft);
    prof_stop(29, prof);
    ...
    if (ret == -2) {
        player_client_exit(nr, "You have been banned. Please email game@astonia.com for details.");
        return;
    }
    if (ret == -3) {
        player_client_exit(nr, "Character name or password wrong.");
        return;
    }
    if (ret == -4) {
        player_client_exit(nr, "Duplicate login. Please make sure no other character from your account is active.");
        return;
    }
    if (ret == -5) {
    ...
    if (ret == -6) {
        player_client_exit(nr, "The server is being shut down. Please try again in a few minutes.");
        return;
    }
    if (ret == -7) {
        player_client_exit(nr, "Your IP address is banned. Please email game@astonia.com with your account ID and ask for an exception to be made.");
        return;
    }
    if (ret == -8) {
        player_client_exit(nr, "Please log onto your account management on www.astonia.com and update the account ownership information. Log in, click on 'Edit Address' and follow the instructions.");
        return;
    }
    if (ret == -9) {
        player_client_exit(nr, "Too many tries with bad passwords. Please come back later.");
        return;
    }
    if (ret == -10) {
        player_client_exit(nr, "The server is full. Please try again later (or pay for your account).");
        return;
    }
    if (ret == -11) {
        int mn, hr, dy;
        ...
        dy = timeleft / (60 * 60 * 24);
        timeleft -= dy * 60 * 60 * 24;
        hr = timeleft / (60 * 60);
        timeleft -= hr * 60 * 60;
        mn = timeleft / 60;
        timeleft -= mn * 60;

        sprintf(buf2, "You have been banned for %d days, %d hours, %d minutes, %d seconds.", dy, hr, mn, timeleft);
        player_client_exit(nr, buf2);
        return;
    }
    if (ret == -12) {
        player_client_exit(nr, "Your account is banned because you accumulated too much Karma.");
        return;
    }
    ...
    *(unsigned int *)(buf + 1) = ch[cn].mirror;
    ...
    if (ch[cn].resta != areaID || abs(ch[cn].x - ch[cn].restx) > 10 || abs(ch[cn].y - ch[cn].resty) > 10) {
        ...
        login_time = realtime - ch[cn].login_time;
        logout_time = realtime - ch[cn].logout_time;

        if (logout_time > 30 && login_time < 15) {
            xlog("zone time: %d secs, kicked, login was %d secs ago", logout_time, login_time);
            exit_char(cn);
            if ((nr = ch[cn].player) && player[nr]) player_client_exit(nr, "Area change was cancelled, please try again.");
        }
    }
}
...
static void cl_container(int nr, char *buf) {
    ...
    if (n < 0 || n >= max(MAXDEPOT, max(STORESIZE, INVENTORYSIZE))) return;
...
static void check_lag(int nr) {
    ...
    if (diff > val && ch[cn].driver != CDR_LOSTCON) {
        log_char(cn, LOG_SYSTEM, 0, "Lag exceeds system limits. Auto-Control taking over. You might want to try to change the setting for /MAXLAG.");
        ch[cn].driver = CDR_LOSTCON;
...
static void cl_junk_item(int nr) {
    ...
    if ((in = ch[cn].citem) && !(it[in].flags & IF_NOJUNK)) {
        if (it[in].value > 25 * 100) {
            log_char(cn, LOG_SYSTEM, 0, "You cannot junk anything worth more than 25G. Please go to a shop to sell it, or drop it on the ground.");
            return;
        }
        tmp = (int)(it[in].value * 0.334);
        dlog(cn, in, "junked %s (%s) for %.2fG", it[in].name, it[in].description, tmp / 100.0);
        if (tmp > 0) give_money(cn, tmp, "junk recycling");
        ch[cn].citem = 0;
        destroy_item(in);
...
static void read_input(int nr) {
    ...
    case CL_JUNK_ITEM:
        ...
        break;
    case CL_CLIENTINFO:
        cl_clientinfo(nr, (void *)(player[nr]->inbuf + 1));
        break;
    case CL_TICKER:
...
static inline int shield_per(int cn) {
    ...
    if (ch[cn].value[0][V_MAGICSHIELD]) from = ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE / MAGICSHIELDMOD;
    else return 0;

    val = ch[cn].lifeshield;

    return max(0, 100 * val / from);
}
...
//#define CCYCLES
    ...
    for (y = 0; y <= DIST * 2; y++) {
        ...
        for (x = xs; x <= xe; x++) {
            ...
            if (x + xoff < 1 || x + xoff >= MAXMAP || y + yoff < 1 || y + yoff >= MAXMAP) {
                flags = 0;
                goto skip;
            }
                ...
                if (isprite != cmap[c].isprite) {
                    ...
                    *(unsigned int *)(buf11 + p11) = cmap[c].isprite = isprite;
                    ...
                    if (isprite & 0x80000000) {
                        *(unsigned short *)(buf11 + p11) = *(unsigned short *)(it[in].drdata + 2);
                        p11 += 2;
                        *(unsigned short *)(buf11 + p11) = *(unsigned short *)(it[in].drdata + 4);
                        p11 += 2;
                        *(unsigned short *)(buf11 + p11) = *(unsigned short *)(it[in].drdata + 6);
                        p11 += 2;
                    }
                }
...
void tick_player(void) {
    ...
    for (n = 1; n < MAXPLAYER; n++) {
        if (player[n]) {
            ...
            default:
                elog("tick_player(): Unknown player state %d for player %d.", player[n]->state, n);
                kick_player(n, NULL);
                break;
            }
        }
