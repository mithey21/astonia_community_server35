 ...
 *
 * $Id: command.c,v 1.42 2008/04/14 14:38:30 devel Exp $
 *
 * $Log: command.c,v $
 * Revision 1.42  2008/04/14 14:38:30  devel
 * /nowarcry and /noshield
 *
 * Revision 1.41  2008/04/11 10:52:33  devel
 * added /showvalues
 *
 * Revision 1.40  2008/03/29 15:45:32  devel
 * added who_tutor()
 *
 * Revision 1.39  2007/12/03 10:07:19  devel
 * added tutor command
 *
 * Revision 1.38  2007/10/29 13:57:45  devel
 * sort bugfix
 *
 * Revision 1.37  2007/10/07 11:18:49  devel
 * stones /sorted buy quality
 * support displays info when called without a name
 *
 * Revision 1.36  2007/08/15 09:13:11  devel
 * removed beta test resetme and setlevel
 *
 * Revision 1.35  2007/08/09 11:11:46  devel
 * beta /setlevel cmd
 *
 * Revision 1.34  2007/07/28 18:50:41  devel
 * paid status switch
 *
 * Revision 1.33  2007/07/27 05:59:32  devel
 * summary statistics
 *
 * Revision 1.32  2007/07/24 18:54:12  devel
 * exterminate needs a reason now
 *
 * Revision 1.31  2007/07/13 15:47:16  devel
 * clog -> charlog
 *
 * Revision 1.30  2007/07/09 11:20:32  devel
 * removed shutup and kick
 *
 * Revision 1.29  2007/07/04 09:20:14  devel
 * added /support support
 *
 * Revision 1.28  2007/07/01 13:26:46  devel
 * removed /allow and helpers
 *
 * Revision 1.27  2007/06/28 12:11:32  devel
 * added /showalts
 *
 * Revision 1.26  2007/06/22 13:02:04  devel
 * added command to create special items
 *
 * Revision 1.25  2007/06/13 15:23:15  devel
 * new punishment system
 *
 * Revision 1.24  2007/06/13 09:26:37  devel
 * added display of gold in bank and payment tokens to /status
 *
 * Revision 1.23  2007/06/12 11:33:42  devel
 * added jump point for new test arena in aston
 *
 * Revision 1.22  2007/06/11 10:04:36  devel
 * items with IF_NOENHANCE set will no longer adjust their value
 *
 * Revision 1.21  2007/06/08 12:07:52  devel
 * added sanity check to /setlevel
 *
 * Revision 1.20  2007/06/07 15:28:54  devel
 * removed PK
 *
 * Revision 1.19  2007/06/07 15:18:29  devel
 * removed CF_NOBLESS
 *
 * Revision 1.18  2007/05/25 12:20:45  devel
 * display area, mirror, online count in /version
 *
 * Revision 1.17  2007/05/04 14:14:49  devel
 * removed old CF_SHUTUP flag
 * added setlevel for testers
 *
 * Revision 1.16  2007/02/26 14:32:15  devel
 * added professions to /reset
 *
 * Revision 1.15  2007/02/24 14:09:54  devel
...
int show_values(int cn, char *name) {
    ...
    sprintf(buf, "%s, level %d, class %s%s%s",
            ch[cn].name,
            ch[cn].level,
            (ch[cn].flags & CF_ARCH) ? "A" : "",
            (ch[cn].flags & CF_WARRIOR) ? "W" : "",
            (ch[cn].flags & CF_MAGE) ? "M" : "");
    tell_chat(0, coID, 0, "%s", buf);

    for (n = 0; n <= V_IMMUNITY; n += 3) {
        sprintf(buf, "%s: %d/%d \010%s: %d/%d \020%s: %d/%d",
                skill[n].name, ch[cn].value[1][n], ch[cn].value[0][n],
                n + 1 <= V_IMMUNITY ? skill[n + 1].name : "none",
                n + 1 <= V_IMMUNITY ? ch[cn].value[1][n + 1] : 0,
                n + 1 <= V_IMMUNITY ? ch[cn].value[0][n + 1] : 0,
                n + 2 <= V_IMMUNITY ? skill[n + 2].name : "none",
                n + 2 <= V_IMMUNITY ? ch[cn].value[1][n + 2] : 0,
                n + 2 <= V_IMMUNITY ? ch[cn].value[0][n + 2] : 0);
        tell_chat(0, coID, 0, "%s", buf);
    }
    sprintf(buf, "%s: %d/%d \010%s: %d/%d \020%s: %d/%d",
            skill[V_DURATION].name, ch[cn].value[1][V_DURATION], ch[cn].value[0][V_DURATION],
            skill[V_RAGE].name, ch[cn].value[1][V_RAGE], ch[cn].value[0][V_RAGE],
            skill[V_PROFESSION].name, ch[cn].value[1][V_PROFESSION], ch[cn].value[0][V_PROFESSION]);
    tell_chat(0, coID, 0, "%s", buf);

    tell_chat(0, coID, 0, "WV: %.2f, AV: %.2f", ch[cn].value[0][V_WEAPON] / 20.0, ch[cn].value[0][V_ARMOR] / 20.0);

    log_char(cn, LOG_SYSTEM, 0, "Sent. Please remember that repeatedly sending your values to another player is considered harassment and will be punished.");
...
int who_tutor(int cn) {
    char buf[80];

    sprintf(buf, "%10d", ch[cn].ID);
    server_chat(1038, buf);
...
static int cmd_complain(int cn, char *ptr) {
    ...
    if (n < 3 || n > 40) ret = -n;
    else ret = lookup_name(name, realname);
...
static int cmd_exterminate(int cn, char *ptr) // 1=OK, 0=repeat
{
    char name[80], reason[80], buf[256], realname[80];
    int n, uID;
    ...
    if (isdigit(*ptr)) {
        log_char(cn, LOG_SYSTEM, 0, "Syntax: exterminate <name> <reason>. Reason must not start with a digit.");
        return 1;
    }
...
static int cmd_punish(int cn, char *ptr) // 1=OK, 0=repeat
{
    ...
    while (isspace(*ptr)) ptr++;
    if (isdigit(*ptr)) {
        log_char(cn, LOG_SYSTEM, 0, "Syntax: punish <name> <reason>. Reason must not start with a digit.");
        return 1;
    }
    for (n = 0; *ptr && n < 79; reason[n++] = *ptr++);
    reason[n] = 0;
...
static int cmd_warn(int cn, char *ptr) // 1=OK, 0=repeat
{
    char name[80], reason[80], buf[256], realname[80];
    int n, uID;
    ...
    while (isspace(*ptr)) ptr++;
    if (isdigit(*ptr)) {
        log_char(cn, LOG_SYSTEM, 0, "Syntax: warn <name> <reason>. Reason must not start with a digit.");
        return 1;
    }
    for (n = 0; *ptr && n < 79; reason[n++] = *ptr++);
    reason[n] = 0;
...
static int cmd_setskill(int cn, char *ptr) // 1=OK, 0=repeat
{
    ...
    if (!pos) {
        for (n = 0; n < V_MAX; n++) {
            if (n == V_SPEED) continue;
            if (n == V_LIGHT) continue;
            if (n == V_ARMOR) continue;
            if (!strncasecmp(skill[n].name, tmp, ptr - tmp)) {
                pos = n;
                break;
            }
        }
...
static int cmd_support(int cn, char *ptr) // 1=OK, 0=repeat
{
    char name[80];
    int n, co, cc;
    ...
    if (n < 2) {
        ...
        if (co) {
            ...
            log_char(cn, LOG_SYSTEM, 0, "Smith at %d.", ch[co].prof[P_SMITH]);
            log_char(cn, LOG_SYSTEM, 0, "Enhancer at %d.", ch[co].prof[P_ENHANCER]);
    ...
    log_char(co, LOG_SYSTEM, 0, "Smith at %d.", ch[cn].prof[P_SMITH]);
    log_char(co, LOG_SYSTEM, 0, "Enhancer at %d.", ch[cn].prof[P_ENHANCER]);
...
static int cmd_staffcode(int cn, char *ptr) // 1=OK, 0=repeat
{
    ...
    while (isspace(*ptr)) ptr++;
    if (isalpha(*ptr)) code[0] = toupper(*ptr++);
    else code[0] = 'A';
    if (isalpha(*ptr)) code[1] = toupper(*ptr++);
    else code[1] = 'A';
    code[2] = 0;
...
static void cmd_reset(int cn, char *ptr) {
    ...
    for (n = 0; n <= V_IMMUNITY; n++) {
        if (n <= V_MANA) {
            if (ch[co].value[1][n] > 25) ch[co].value[1][n] = 25;
        } else if (n <= V_STR) {
    ...
    if (ch[co].value[1][V_PROFESSION] > 1) ch[co].value[1][V_PROFESSION] = 1;

    for (n = 0; n < P_MAX; n++) ch[co].prof[n] = 0;
...
int sort_cmp(const void *a, const void *b) {
    ...
    if (it[*(int *)(a)].ID == IID_ALCHEMY_INGREDIENT &&
        it[*(int *)(b)].ID == IID_ALCHEMY_INGREDIENT &&
        it[*(int *)(a)].complexity > 0 &&
        it[*(int *)(b)].complexity > 0) {
        qual_a = it[*(int *)(a)].quality * 100 / it[*(int *)(a)].complexity;
        qual_b = it[*(int *)(b)].quality * 100 / it[*(int *)(b)].complexity;
...
static void cmd_help(int cn) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "/version - show server version and estimated online count");
    ...
    if (ch[cn].flags & CF_STAFF) {
        ...
        log_char(cn, LOG_SYSTEM, 0, "/punish <name> <reason> - timed ban and karma");
        log_char(cn, LOG_SYSTEM, 0, "/nowho");
        log_char(cn, LOG_SYSTEM, 0, "/values <name>");
        log_char(cn, LOG_SYSTEM, 0, "/warn <name> <reason> - text warning");
    }
    ...
    if (ch[cn].flags & CF_GOD) {
        ...
        log_char(cn, LOG_SYSTEM, 0, "/setlevel <lvl>");
        log_char(cn, LOG_SYSTEM, 0, "/setskill <name> <idx> <val>");
    }
}
...
void cmd_renclan(int cn, char *ptr) {
    ...
    if (areaID != 13) {
        log_char(cn, LOG_SYSTEM, 0, "Sorry, this command only works in the Clan Area.");
        return;
    }
...
int command(int cn, char *ptr) // 1=ok, 0=repeat
{
    ...
    if ((len = cmdcmp(ptr, "immortal", 2)) && is_god(cn)) {
    ...
    if ((len = cmdcmp(ptr, "infrared", 3)) && is_god(cn)) {
    ...
    if ((len = cmdcmp(ptr, "invisible", 3)) && is_god(cn)) {
    ...
    if ((len = cmdcmp(ptr, "showspell", 6)) && (ch[cn].flags & CF_GOD)) {
        showspell = 1 - showspell;
        return 1;
    }
    if ((len = cmdcmp(ptr, "showhurt", 6)) && (ch[cn].flags & CF_GOD)) {
        showhurt = 1 - showhurt;
        return 1;
    }
    ...
    if ((len = cmdcmp(ptr, "joinclan", 8)) && (ch[cn].flags & CF_GOD)) {
        ...
        if (nr >= 0 && nr < MAXCLAN) {
            if (ch[cn].clan) ch[cn].clan_leave_date = realtime;
            ch[cn].clan = nr;
    ...
    if ((len = cmdcmp(ptr, "killclan", 8)) && (ch[cn].flags & CF_GOD)) {
        ...
        if (areaID != 13) {
            log_char(cn, LOG_SYSTEM, 0, "Sorry, this command works only in the clan area.");
            return 1;
        }
    ...
    if ((len = cmdcmp(ptr, "warn", 4)) && (ch[cn].flags & (CF_GOD | CF_STAFF))) {
        ptr += len;
        while (isspace(*ptr)) ptr++;

        return cmd_warn(cn, ptr);
    }
    ...
    if ((len = cmdcmp(ptr, "showvalues", 6))) {
        ptr += len;
    ...
    if ((len = cmdcmp(ptr, "whotutor", 4)) && (ch[cn].flags & (CF_GOD | CF_STAFF | CF_TUTOR))) {
        who_tutor(cn);
        return 1;
    }

    if ((len = cmdcmp(ptr, "goto", 3)) && is_god(cn)) {
        int x, y, a = 0, m;
    ...
    if ((len = cmdcmp(ptr, "special", 7)) && (ch[cn].flags & CF_GOD)) {
        int in, val;
        ...
        if ((in = create_special_item(7, 1, val))) {
    ...
    if ((len = cmdcmp(ptr, "look", 4)) && (ch[cn].flags & (CF_GOD | CF_STAFF))) {
        ...
        readnotes(ch[cn].ID, ID);

        return 1;
    }

    if ((len = cmdcmp(ptr, "support", 3))) {
    ...
    if (cmdcmp(ptr, "nosupport", 5)) {
        ...
        log_char(cn, LOG_SYSTEM, 0, "Support canceled.");
        ch[cn].supported = 0;
        ch[cn].supporting = 0;
    ...
    if ((len = cmdcmp(ptr, "showalts", 8)) && (ch[cn].flags & (CF_GOD))) {
        char name[80] = {"oops"};
        int ID;
    ...
    if ((len = cmdcmp(ptr, "god", 3)) && ((ch[cn].flags & CF_GOD) || ch[cn].ID == 1)) {
        ptr += len;
        while (isspace(*ptr)) ptr++;
    ...
    if ((len = cmdcmp(ptr, "tutor", 5)) && (ch[cn].flags & CF_GOD)) {
        ptr += len;
        while (isspace(*ptr)) ptr++;

        return cmd_flag(cn, ptr, CF_TUTOR);
    }
    ...
    if ((len = cmdcmp(ptr, "version", 4))) {
        ...
        log_char(cn, LOG_SYSTEM, 0, "Server Version %d.%d Build %4d (compiled %s %s)", VERSION >> 24, (VERSION >> 16) & 255, VERSION & 65535, VER_TIME, VER_DATE);
    ...
    if (cmdcmp(ptr, "nowarcry", 5)) {
        if ((ppd = set_data(cn, DRD_LOSTCON_PPD, sizeof(struct lostcon_ppd)))) {
            if (ppd->nowarcry) ppd->nowarcry = 0;
            else ppd->nowarcry = 1;
            show_lostconppd(cn, ppd);
        }
        return 1;
    ...
    if (cmdcmp(ptr, "noshield", 5)) {
        if ((ppd = set_data(cn, DRD_LOSTCON_PPD, sizeof(struct lostcon_ppd)))) {
            if (ppd->noshield) ppd->noshield = 0;
            else ppd->noshield = 1;
            show_lostconppd(cn, ppd);
        }
        return 1;
    ...
    if (cmdcmp(ptr, "norecall", 5)) {
        if ((ppd = set_data(cn, DRD_LOSTCON_PPD, sizeof(struct lostcon_ppd)))) {
            if (ppd->norecall) ppd->norecall = 0;
            else ppd->norecall = 1;
            show_lostconppd(cn, ppd);
        }
        return 1;
    ...
    if (cmdcmp(ptr, "autoturn", 5)) {
        if ((ppd = set_data(cn, DRD_LOSTCON_PPD, sizeof(struct lostcon_ppd)))) {
            if (ppd->autoturn) ppd->autoturn = 0;
            else ppd->autoturn = 1;
            show_lostconppd(cn, ppd);
        }
        return 1;
    ...
    if ((len = cmdcmp(ptr, "settoken", 8)) && (ch[cn].flags & (CF_GOD))) {
        int cnt;
        struct depot_ppd *depot_ppd;
        ...
        if (depot_ppd && depot_ppd->loaded) {
            depot_ppd->payment_tokens = cnt;
            depot_ppd->changes++;
        }
