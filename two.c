...
void guard_parse(int cn, struct twoguard_data *dat) {
    ...
    for (ptr = nextnv(ch[cn].arg, name, value); ptr; ptr = nextnv(ptr, name, value)) {
        ...
        if (!strcmp(name, "patx")) {
            if (pat >= MAXPAT) elog("twoguard_parse: too many patrol stops");
            else dat->patx[pat] = atoi(value);
        } else if (!strcmp(name, "paty")) {
            if (pat >= MAXPAT) elog("twoguard_parse: too many patrol stops");
            else dat->paty[pat++] = atoi(value);
...
void guard_driver(int cn, int ret, int lastact) {
    ...
    if (spell_self_driver(cn)) {
        dat->busy = 1;
        return;
    }
    if (regenerate_driver(cn)) {
        dat->busy = 1;
        return;
    }
    ...
    if (dat->tx) {
        ...
        else if (swap_move_driver(cn, dat->tx, dat->ty, 1)) {
            dat->good_tx_try = ticker;
            return;
        } else if (swap_move_driver(cn, dat->tx, dat->ty, 3)) {
            dat->good_tx_try = ticker;
            return;
        } else if (ticker - dat->good_tx_try > TICKS * 10) {
            dat->tx = dat->ty = 0;
        }
    }
    ...
    if (ch[cn].tmpx - ch[cn].x == 0 && ch[cn].tmpy - ch[cn].y == 0) {
        do_idle(cn, TICKS);
...
void barkeeper(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (realtime - ppd->barkeeper_last > 60 * 10) ppd->barkeeper_state = 2;
                    break;
                }
                if (didsay) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 2:
                ppd = set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
                if (ppd && ppd->barkeeper_state <= 2) {
                    dat->last_talk = 0;
                    ppd->barkeeper_state = 0;
                }
                break;
            case 13:
                ppd = set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 8: //chat
                ...
                case 0:
                    ...
                    break;
                case 1:
                    say(cn, "The governor, he is a cruel man. Do be very careful here in Exkordon.");
                    didsay = 1;
                    break;
                case 2:
                    say(cn, "I am the governor's mistress. Oh, I wish I could leave him, oh, I wish.");
                    didsay = 1;
                    break;
                case 3:
                    ...
                    break;
                case 5:
                    if (hour > 6 && hour < 23) say(cn, "So nice of thee to come and visit, %s. Life has been dull, and thou art most interesting.", ch[co].name);
            ...
            case 10: // threaten
                ...
                case 0:
                    ...
                    break;
                case 1:
                    say(cn, "I shall tell the guards about thee, %s. Now go.", ch[co].name);
                    call_guard(cn, co);
                    didsay = 1;
                    break;
                case 2:
            ...
            case 11: // pay bribe
                switch (dat->nr) {
                case 0:
                    if (take_money(co, 2000)) say(cn, "Ooh. I thank thee, noble %s, I thank thee! But wait. There is one thing I can tell thee: Avoid the governors study at all cost. It is behind the door leading south-east from the small hallway.", Sirname(co));
...
void pick_chest(int in, int cn) {
    ...
    case 0:
        in2 = create_item("palace_note1");
        break;
    case 1:
        in2 = create_item("palace_note2");
        break;
    case 2:
        in2 = create_item("palace_note3");
        break;
    case 3:
        in2 = create_item("merchant_note1");
        break;
    default:
        in2 = 0;
        break;
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    say(cn, "HALT! Who's there? Ah, a stranger in our wonderful town. Hello, %s, and welcome to the thieves guild. Thou mayest not enter...", ch[co].name);
                    ppd->thief_state++;
                    didsay = 1;
                    break;
                case 1:
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                switch (ppd->thief_state) {
                case 4:
                    say(cn, "Ah. A new member. Welcome, %s.", ch[co].name);
                    ppd->thief_state++;
                    didsay = 1;
                    break;
                case 5:
                ...
                case 12:
                    ...
                    break;
                case 13:
                    break; // waiting for barrels to burn down
                case 14:
                    score = ppd->thief_killed[0];
                    if (score < 10) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 2:
                ppd = set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
                if (ppd && ppd->thief_state <= 9) {
                    dat->last_talk = 0;
                    ppd->thief_state = 4;
                }
                if (ppd && ppd->thief_state >= 11 && ppd->thief_state <= 13) {
                    dat->last_talk = 0;
                    ppd->thief_state = 11;
                }
                if (ppd && ppd->thief_state >= 15 && ppd->thief_state <= 17) {
                    dat->last_talk = 0;
                    ppd->thief_state = 15;
                }
                if (ppd && ppd->thief_state >= 18 && ppd->thief_state <= 19) {
                    dat->last_talk = 0;
                    ppd->thief_state = 18;
                }
                if (ppd && ppd->thief_state >= 20 && ppd->thief_state <= 21) {
                    dat->last_talk = 0;
                    ppd->thief_state = 20;
                }
                break;
...
void robber_dead(int cn, int co) {
    ...
    if (ppd && ppd->thief_state >= 6 && ppd->thief_state <= 9) {
        ...
        case 52:
            ...
            break;
        default:
            elog("unlisted robber level %d in two.c, robber_dead()", ch[cn].level);
            break;
        }
    }
}
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    say(cn, "Welcome, %s, to the noble and honorable city of Exkordon. I am %s, Sergeant Major in the Imperial Army.", ch[co].name, ch[cn].name);
                    questlog_open(co, 29);
                    ppd->sanwyn_state++;
                    didsay = 1;
                    break;
                ...
                case 4:
                    ...
                    break;
                case 5:
                    say(cn, "That will be all, %s. May thy stay in Exkordon be... destructive.", get_army_rank_string(co));
                    ppd->sanwyn_state++;
                    didsay = 1;
                    break;
                case 6:
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 2:
                ppd = set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
                if (ppd && ppd->sanwyn_state <= 6) {
                    dat->last_talk = 0;
                    ppd->sanwyn_state = 0;
                }
                if (ppd && ppd->sanwyn_state == 8) {
                    dat->last_talk = 0;
                    ppd->sanwyn_state = 7;
                }
                break;
...
void bookcase(int in, int cn) {
    ...
    if (!it[in].drdata[0]) { // random books to hide the important ones
        ...
        switch (r) {
        case 0:
            name = "Tales of Two Towns by Karl Dicker";
            break;
        case 1:
            name = "The Art of Warfare by Hun Yu";
            break;
        case 2:
            name = "Chris Maas visits Carol by Karl Dicker";
            break;
        case 3:
            name = "Secrets of Adygalah Alchemy by Leonarda";
            text = "One recipe most mages will find useful uses Adygalah, Bhalkissa and Firuba, plus one berry and one or two mushrooms.";
            break;
        case 4:
            name = "The rise and fall of the Seyan Empire by Takitus";
            break;
        case 5:
            name = "History of Ancient Astonia by Chiasmaphora";
            break;
        case 6:
            name = "Treatise on the Mastery of Mana by Mage Niuma";
            break;
        case 7:
            name = "The Song of the Warrior by Sir Regis Le Voleir";
            break;
        case 8:
            name = "The Book of Ishtar, Anonymous";
            break;
        case 9:
            name = "Concessions to Fear by Kentindher";
            break;
        case 10:
            name = "Poems of War and Homecoming by Melthold of Anten";
            break;
        case 11:
            name = "Memoires of a Lady-in-Waiting by Dame Sakanor";
            break;
        case 12:
            name = "Comprehension and Expression by Master Getsades";
            break;
        case 13:
            name = "Great Astonian Thinkers by Master Riotan";
            break;
        case 14:
            name = "A Portrait of the Seyan'Du as A Young Mage by Esjamocey";
            break;
        case 15:
            name = "Critique of Pure Courage by Imanel Dique";
            break;
        case 16:
            name = "Collected Essays by Lindmar the Elder";
            break;
        case 17:
            name = "The Reforming of Curves by Master Elyosod";
            break;
        case 18:
            name = "Advanced Agility in Forty-two Steps by Seyan'Du Bartoshi";
            break;
        case 19:
            name = "The Oath by Sheney";
            break;
        case 20:
            name = "The Strife for Light by Father Ignato";
            break;
        case 21:
            name = "The Aston Years by Lord Ironborn";
            break;
        case 22:
            name = "Luctim - Superstition or Reality? by Mintu the Enlightened";
            break;
        case 23:
            name = "I Have, Alas by Goytila";
            break;
        case 24:
            name = "A Midwinter Day's Wake by Pearshaks";
            break;
        case 25:
            name = "Fama Fraternitatis by Valentin Andreae";
            break;
        }
    } else { // special books
        switch (it[in].drdata[0]) {
        case 1:
            if (!has_item(cn, IID_AREA17_LIBRARYKEY)) {
            ...
            break;
        case 2:
            sprintf(buf, "How to Raise %s Orchids by Klark", colorname[ppd->goodtile[0]]);
            name = buf;
            break;
        case 3:
            sprintf(buf, "A %s Day in the Life of a Warrior by C. O. Nan", colorname[ppd->goodtile[1]]);
            name = buf;
            break;
        case 4:
            sprintf(buf, "Dancing in Ten Easy Lessons by James %s", colorname[ppd->goodtile[2]]);
            name = buf;
            break;
        case 5:
            sprintf(buf, "Help! I Have Been Visited by Little %s Man! by Meier", colorname[ppd->goodtile[3]]);
            name = buf;
            break;
        case 6:
            sprintf(buf, "The Day the World turned %s by Casaldra", colorname[ppd->goodtile[4]]);
            name = buf;
            break;
        }
    }
...
void skelraise(int in, int cn) {
    ...
    if (!(in2 = ch[cn].citem) || it[in2].ID != IID_AREA17_BLOODBOWL) {
        ...
        *(unsigned *)(it[in].drdata + 8) = 0;
        ...
        switch (it[in].drdata[0]) {
        case 0:
            co = create_drop_char("raised_skeleton_green", it[in].x, it[in].y);
            *(unsigned *)(it[in].drdata + 4) = co;
            *(unsigned *)(it[in].drdata + 8) = ch[co].serial;
            break;
        case 1:
            co = create_drop_char("raised_skeleton_red", it[in].x, it[in].y);
            *(unsigned *)(it[in].drdata + 4) = co;
            *(unsigned *)(it[in].drdata + 8) = ch[co].serial;
            break;
        case 2:
            co = create_drop_char("raised_skeleton_green_key", it[in].x, it[in].y);
            *(unsigned *)(it[in].drdata + 4) = co;
            *(unsigned *)(it[in].drdata + 8) = ch[co].serial;
            break;
        case 3:
            co = create_drop_char("raised_skeleton_red_key", it[in].x, it[in].y);
            *(unsigned *)(it[in].drdata + 4) = co;
            *(unsigned *)(it[in].drdata + 8) = ch[co].serial;
            break;
        case 4:
            co = create_drop_char("raised_skeleton_nolight", it[in].x, it[in].y);
            *(unsigned *)(it[in].drdata + 4) = co;
            *(unsigned *)(it[in].drdata + 8) = ch[co].serial;
            break;
        case 5:
            co = create_drop_char("quest_skeleton", it[in].x, it[in].y);
            *(unsigned *)(it[in].drdata + 4) = co;
            *(unsigned *)(it[in].drdata + 8) = ch[co].serial;
            break;
        }
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    say(cn, "My greetings, %s. I am %s, Governor of Exkordon. Former governor, I should say.", ch[co].name, ch[cn].name);
                    ppd->skelly_state++;
                    didsay = 1;
                    questlog_open(co, 30);
                    break;
                case 1:
                    emote(cn, "speaks in a different voice now");
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    say(cn, "Too much sulphur. Yes, too much sulphur. Oh, hello, %s. I am %s, the alchemist.", ch[co].name, ch[cn].name);
                    questlog_open(co, 31);
                    ppd->alchemist_state++;
                    didsay = 1;
                    break;
                case 1:
                    ...
                    break;
                case 2:
                    say(cn, "I know of some poisonous spiders who live underground, in a plantage the ancients built to grow food. When I went there some years ago, the lighting was beginning to fail. Anyway. The entrance is close to the transport portal.");
                    ppd->alchemist_state++;
                    didsay = 1;
                    break;
                case 3:
                    say(cn, "Thou wilt need to look for bright red spiders, those are the only ones who have the poison I need.");
                    ppd->alchemist_state++;
                    didsay = 1;
                    break;
                case 4:
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_TWOGUARD:
        guard_driver(cn, ret, lastact);
        return 1;
    case CDR_TWOBARKEEPER:
        barkeeper(cn, ret, lastact);
        return 1;
    case CDR_TWOSERVANT:
        servant(cn, ret, lastact);
        return 1;
    case CDR_TWOTHIEFGUARD:
        thiefguard(cn, ret, lastact);
        return 1;
    case CDR_TWOTHIEFMASTER:
        thiefmaster(cn, ret, lastact);
        return 1;
    case CDR_TWOROBBER:
        char_driver(CDR_SIMPLEBADDY, CDT_DRIVER, cn, ret, lastact);
        return 1;
    case CDR_TWOSANWYN:
        sanwyn(cn, ret, lastact);
        return 1;
    case CDR_TWOSKELLY:
        skelly(cn, ret, lastact);
        return 1;
    case CDR_TWOALCHEMIST:
        alchemist(cn, ret, lastact);
        return 1;
