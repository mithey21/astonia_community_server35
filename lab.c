...
static int teleport_lab(int cn, int lab_level, int do_teleport) {
    switch (lab_level) {
    case 10:
        if (ch[cn].level < 10) return -10;
        if (!do_teleport || change_area(cn, 22, 27, 242)) return 1;
        else return -1;
    case 15:
        if (ch[cn].level < 12) return -12;
        if (!do_teleport || change_area(cn, 22, 69, 105)) return 1;
        else return -1;
    case 20:
        if (ch[cn].level < 15) return -15;
        if (!do_teleport || change_area(cn, 22, 227, 250)) return 1;
        else return -1;
    case 25:
        if (ch[cn].level < 20) return -20;
        if (!do_teleport || change_area(cn, 22, 144, 103)) return 1;
        else return -1;
    case 30:
        if (ch[cn].level < 25) return -25;
        if (!do_teleport || change_area(cn, 22, 163, 243)) return 1;
        else return -1;
    default:
        return 0;
    }
