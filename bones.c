...
void exec_rune(int cn, int nr, int lastholder) {
    ...
    // level 1
    case 1:
        teleport_char_driver(cn, 90, 25);
        break; // teleport to next area
    case 11:
        give_exp(cn, level_value(min(ch[cn].level + 5, 50)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 111:
        log_char(cn, LOG_SYSTEM, 0, "You hear a sinister laugh echo through the emptiness.");
        teleport_char_driver(cn, 72, 52);
        break;

    // level 2
    case 2:
        teleport_char_driver(cn, 175, 4);
        break; // teleport to next area
    case 22:
        give_exp(cn, level_value(min(ch[cn].level + 5, 51)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 222:
        ...
        break;
    case 212:
        if (raise_value_exp(cn, V_ENDURANCE)) log_char(cn, LOG_SYSTEM, 0, "You gained endurance.");
        flag = 1;
        break;

    // level 3
    case 3:
        teleport_char_driver(cn, 4, 69);
        break; // teleport to next area
    case 33:
        give_exp(cn, level_value(min(ch[cn].level + 5, 52)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 333:
        ...
        break;
    case 231:
        if (raise_value_exp(cn, V_HP)) log_char(cn, LOG_SYSTEM, 0, "You gained hitpoints.");
        flag = 1;
        break;
    case 133:
        if (raise_value_exp(cn, V_MANA)) log_char(cn, LOG_SYSTEM, 0, "You gained mana.");
        flag = 1;
        break;

    // level 4
    case 4:
        teleport_char_driver(cn, 90, 95);
        break; // teleport to next area
    case 44:
        give_exp(cn, level_value(min(ch[cn].level + 5, 54)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 444:
        ...
        break;
    case 143:
        if (raise_value_exp(cn, V_PARRY)) log_char(cn, LOG_SYSTEM, 0, "You gained parry.");
        flag = 1;
        break;
    case 442:
        if (raise_value_exp(cn, V_MAGICSHIELD)) log_char(cn, LOG_SYSTEM, 0, "You gained magic shield.");
        flag = 1;
        break;
    case 241:
        if (raise_value_exp(cn, V_IMMUNITY)) log_char(cn, LOG_SYSTEM, 0, "You gained immunity.");
        flag = 1;
        break;

    // level 5
    case 5:
        teleport_char_driver(cn, 176, 110);
        break; // teleport to next area
    case 55:
        give_exp(cn, level_value(min(ch[cn].level + 5, 55)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 555:
    ...
    // level 6
    case 6:
        teleport_char_driver(cn, 4, 146);
        break; // teleport to next area
    case 66:
        give_exp(cn, level_value(min(ch[cn].level + 5, 57)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 666:
    ...
    // level 7
    case 7:
        teleport_char_driver(cn, 91, 171);
        break; // teleport to next area
    case 77:
        give_exp(cn, level_value(min(ch[cn].level + 5, 58)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 777:
    ...
    // level 8
    case 8:
        teleport_char_driver(cn, 187, 152);
        break; // teleport to next area
    case 88:
        give_exp(cn, level_value(min(ch[cn].level + 5, 59)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 888:
    ...
    // level 9
    case 9:
        teleport_char_driver(cn, 137, 222);
        break; // teleport to next area
    case 99:
        give_exp(cn, level_value(min(ch[cn].level + 5, 61)) / 6);
        flag = 1; // bonus
        log_char(cn, LOG_SYSTEM, 0, "You gained experience.");
        break;
    case 999:
    ...
    default:
        for (n = 0; n < 25; n++) {
            if (ppd->special_exec[n] == nr) {
                ...
                // level 5
                case 0:
                    rune_raise(cn, V_ATTACK, V_FLASH);
                    flag = 1;
                    break;
                case 1:
                    rune_raise(cn, V_SWORD, V_FREEZE);
                    flag = 1;
                    break;
                case 2:
                    rune_raise(cn, V_TWOHAND, V_FIRE);
                    flag = 1;
                    break;
                case 3:
                    rune_raise(cn, V_HAND, V_HEAL);
                    flag = 1;
                    break;
                case 4:
                    rune_raise(cn, V_PERCEPT, V_PERCEPT);
                    flag = 1;
                    break;

                // level 6
                case 5:
                    rune_raise(cn, V_PROFESSION, V_PROFESSION);
                    flag = 1;
                    break;
                case 6:
                    rune_raise(cn, V_INT, V_INT);
                    flag = 1;
                    break;
                case 7:
                    rune_raise(cn, V_STR, V_WIS);
                    flag = 1;
                    break;
                case 8:
                    rune_raise(cn, V_HP, V_MANA);
                    flag = 1;
                    break;
                case 9:
                    rune_raise(cn, V_STEALTH, V_STEALTH);
                    flag = 1;
                    break;
                ...
                case 11:
                    ...
                    break;
                case 12:
                    rune_raise(cn, V_PARRY, V_MAGICSHIELD);
                    flag = 1;
                    break;
                case 13:
                    rune_raise(cn, V_SURROUND, V_DAGGER);
                    flag = 1;
                    break;
                case 14:
                    rune_raise(cn, V_SPEEDSKILL, V_STAFF);
                    flag = 1;
                    break;

                // level 8
                case 15:
                    rune_raise(cn, V_TACTICS, V_BLESS);
                    flag = 1;
                    break;
                case 16:
                    rune_raise(cn, V_AGI, V_AGI);
                    flag = 1;
                    break;
                case 17:
                    rune_raise(cn, V_HP, V_MANA);
                    ...
                    break;
                case 18:
                    rune_raise(cn, V_ENDURANCE, V_ENDURANCE);
                    flag = 1;
                    break;
                case 19:
                    rune_raise(cn, V_BARTER, V_BARTER);
                    flag = 1;
                    break;

                // level 9
                case 20:
                    if (!lastholder) log_char(cn, LOG_SYSTEM, 0, "This combination seems to be right, but it does not work here.");
                    else {
                        teleport_char_driver(cn, 14, 213);
                        log_char(cn, LOG_SYSTEM, 0, "Uh-oh");
                    }
                ...
                case 21:
                    ...
                    break;
                case 22:
                    rune_raise(cn, V_IMMUNITY, V_IMMUNITY);
                    flag = 1;
                    break;
                case 23:
                    rune_raise(cn, V_PROFESSION, V_PROFESSION);
                ...
                default:
                    log_char(cn, LOG_SYSTEM, 0, "You found bug #1926");
                    break;
                }
                break;
            }
