...
void palace_guard_parse(int cn, struct palace_guard_data *dat) {
    ...
    for (ptr = nextnv(ch[cn].arg, name, value); ptr; ptr = nextnv(ptr, name, value)) {
        ...
        if (!strcmp(name, "patrolx")) {
            if (pat >= MAXPAT) elog("palace_guard_parse: too many patrol stops");
            else dat->patrolx[pat] = atoi(value);
        } else if (!strcmp(name, "patroly")) {
            if (pat >= MAXPAT) elog("palace_guard_parse: too many patrol stops");
            else dat->patroly[pat++] = atoi(value);
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if ((ch[co].flags & CF_PLAYER) && char_see_char(cn, co) && (ppd = set_data(co, DRD_ISLENA_PPD, sizeof(struct islena_ppd)))) {
                ...
                case 0:
                    say(cn, "Ah, %s. I have heard of thee. Thou hast quite a reputation among my minions.", ch[co].name);
                    ppd->islena_state++;
                    didsay = 1;
                    break;
                case 1:
                    say(cn, "Thou hast been led by the nose by Ishtar, %s. I was quite content living here among my creatures, but Ishtar had to stir up hatred among the human population against me.", ch[co].name);
                    ppd->islena_state++;
                    didsay = 1;
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    say(cn, "None of us must die today if thou wilt just leave me be, %s.", ch[co].name);
                    ppd->islena_state++;
                    didsay = 1;
                    break;
                }
