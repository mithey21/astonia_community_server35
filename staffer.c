...
void staffer_item(int in, int cn) {
    ...
    case 1:
        staffer_spiketrap(in, cn);
        break;
    case 2:
        staffer_fireball_machine(in, cn);
        break;
    case 3:
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    quiet_say(cn, "Greetings, %s!", ch[co].name);
                    questlog_open(co, 35);
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (ch[co].flags & CF_PLAYER) {
                ...
                case 2:
                    if (ppd && ppd->smugglecom_state <= 4) {
                        dat->last_talk = 0;
                        ppd->smugglecom_state = 0;
                    }
                    if (ppd && ppd->smugglecom_state >= 5 && ppd->smugglecom_state <= 6) {
                        dat->last_talk = 0;
                        ppd->smugglecom_state = 5;
                    }
                    if (ppd && ppd->smugglecom_state >= 7 && ppd->smugglecom_state <= 8) {
                        dat->last_talk = 0;
                        ppd->smugglecom_state = 7;
                    }
                    break;
                case 3:
                    if (ch[co].flags & CF_GOD) ppd->smugglecom_bits = ppd->smugglecom_state = 0;
                    break;
                }
                if (didsay) {
                    talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_SMUGGLECOM:
        smugglecom_driver(cn, ret, lastact);
        return 1;
    case CDR_SMUGGLELEAD:
