...
int ruby_create_item(struct ruby_aston_npc *me) {
    ...
    case V_TWOHAND:
        sprite = 10373;
        dam = 12;
        flags = IF_TWOHAND | IF_WNTWOHANDED;
        name = "Two-Handed Sword";
        desc = "two-handed sword";
        break;
    case V_SWORD:
        sprite = 10384;
        dam = 6;
...
int ruby_aston_char(int cn, int co, struct sidestory_ppd *ppd) {
    ...
    case 0:
        if (bitvalue >= 30) ppd->ruby3.state = 3;
    ...
    case 2:
        ...
        break;
    case 3:
        quiet_say(cn, "Oh, hello again, %s. I guess I owe thee something for thine help. I collect weapons of all kinds. Maybe thou'd take one of them for all thy trouble?", ch[co].name);
        ppd->ruby3.state++;
        didsay = 1;
        break;
    case 4:
    ...
    case 5:
        ...
        else if (ppd->ruby3.skl == V_TWOHAND) quiet_say(cn, "I guess I have some two-handed weapons somewhere.");
        else quiet_say(cn, "I guess I have some %ss somewhere.", skill[ppd->ruby3.skl].name);
        ppd->ruby3.state++;
        didsay = 1;
        break;
    case 6:
    ...
    case 8:
        ...
        if (ppd->ruby3.mod1 < 0 || ppd->ruby3.mod1 >= V_MAX) {
            ppd->ruby3.state = 0;
            break;
        }
    ...
    case 10:
        ...
        if (ppd->ruby3.mod2 < 0 || ppd->ruby3.mod2 >= V_MAX) {
            ppd->ruby3.state = 0;
            break;
        }
    ...
    case 11:
        ...
        else if (ch[co].level < 27) val = 6;
        else if (ch[co].level < 31) val = 7;
        else if (ch[co].level < 35) val = 8;
        else if (ch[co].level < 40) val = 9;
        else if (ch[co].level < 44) val = 10;
        else if (ch[co].level < 48) val = 11;
        else if (ch[co].level < 53) val = 12;
        else if (ch[co].level < 57) val = 13;
        else if (ch[co].level < 61) val = 14;
        else if (ch[co].level < 66) val = 15;
        else if (ch[co].level < 70) val = 16;
        else if (ch[co].level < 74) val = 17;
        else if (ch[co].level < 79) val = 18;
        else if (ch[co].level < 83) val = 19;
        else val = 20;
    ...
    case 12:
        break;
    case 13:
        quiet_say(cn, "Oh. Alright. Feel free to visit me again, I might have something different for thee then. Just say 'repeat'.");
        ppd->ruby3.state++;
        didsay = 1;
        break;
    case 14:
...
void ruby_aston_text(int cn, int co, struct sidestory_ppd *ppd, char *ptr) {
    while (*ptr && *ptr != ':') ptr++;
    if (!*ptr) return;
    if (strcasestr(ptr, "repeat")) {
        ppd->ruby3.state = 0;
        ppd->ruby3.last_say = 0;
        return;
    }
    ...
    if (ppd->ruby3.state == 7 && strcasestr(ptr, "tactics")) {
        ppd->ruby3.mod1 = V_TACTICS;
        ppd->ruby3.state = 8;
        ppd->ruby3.last_say = 0;
        return;
    }
    if (ppd->ruby3.state == 7 && strcasestr(ptr, "bless")) {
        ppd->ruby3.mod1 = V_BLESS;
        ppd->ruby3.state = 8;
        ppd->ruby3.last_say = 0;
        return;
    }
    if (ppd->ruby3.state == 7 && strcasestr(ptr, skill[ppd->ruby3.skl].name)) {
        ppd->ruby3.mod1 = ppd->ruby3.skl;
        ppd->ruby3.state = 8;
        ppd->ruby3.last_say = 0;
        return;
    }

    if (ppd->ruby3.state == 9 && strcasestr(ptr, "wisdom")) {
        ppd->ruby3.mod2 = V_WIS;
        ppd->ruby3.state = 10;
        ppd->ruby3.last_say = 0;
        return;
    }
    if (ppd->ruby3.state == 9 && strcasestr(ptr, "intuition")) {
        ppd->ruby3.mod2 = V_INT;
        ppd->ruby3.state = 10;
        ppd->ruby3.last_say = 0;
        return;
    }
    if (ppd->ruby3.state == 9 && strcasestr(ptr, "agility")) {
        ppd->ruby3.mod2 = V_AGI;
        ppd->ruby3.state = 10;
        ppd->ruby3.last_say = 0;
        return;
    }
    if (ppd->ruby3.state == 9 && strcasestr(ptr, "strength")) {
        ppd->ruby3.mod2 = V_STR;
        ppd->ruby3.state = 10;
        ppd->ruby3.last_say = 0;
        return;
    }

    if (ppd->ruby3.state == 12 && strcasestr(ptr, "be great")) {
        ppd->ruby3.state = 15;
        ppd->ruby3.last_say = 0;
        return;
    }
    if (ppd->ruby3.state == 12 && strcasestr(ptr, "thanks anyway")) {
        ppd->ruby3.state = 13;
        ppd->ruby3.last_say = 0;
        return;
    }
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if ((ch[co].flags & CF_PLAYER) &&
                ...
                switch (areaID) {
                case 2:
                    ret = do_story(cn, co, ppd, &ppd->ruby2, ruby2_story);
                    break;
                case 3:
                    ret = ruby_aston_char(cn, co, ppd);
                    break;
                case 5:
                    ret = do_story(cn, co, ppd, &ppd->ruby5, ruby5_story);
                    break;
                case 6:
                    ret = do_story(cn, co, ppd, &ppd->ruby6, ruby6_story);
                    break;
                case 8:
                    ret = do_story(cn, co, ppd, &ppd->ruby8, ruby8_story);
                    break;
                case 10:
                    ret = do_story(cn, co, ppd, &ppd->ruby10, ruby10_story);
                    break;
                case 11:
                    ret = do_story(cn, co, ppd, &ppd->ruby11, ruby11_story);
                    break;
                case 12:
                    ret = do_story(cn, co, ppd, &ppd->ruby12, ruby12_story);
                    break;
                case 15:
                    ret = do_story(cn, co, ppd, &ppd->ruby15, ruby15_story);
                    break;
                case 16:
                    ret = do_story(cn, co, ppd, &ppd->ruby16, ruby16_story);
                    break;
                case 17:
                    ret = do_story(cn, co, ppd, &ppd->ruby17, ruby17_story);
                    break;
        ...
        if (msg->type == NT_TEXT) {
            ...
            if ((ch[co].flags & CF_PLAYER) &&
                ...
                switch (areaID) {
                case 2:
                    hear_story(cn, co, ppd, &ppd->ruby2, ruby2_story, ptr);
                    break;
                case 3:
                    ruby_aston_text(cn, co, ppd, ptr);
                    break;
                case 5:
                    hear_story(cn, co, ppd, &ppd->ruby5, ruby5_story, ptr);
                    break;
                case 6:
                    hear_story(cn, co, ppd, &ppd->ruby6, ruby6_story, ptr);
                    break;
                case 8:
                    hear_story(cn, co, ppd, &ppd->ruby8, ruby8_story, ptr);
                    break;
                case 10:
                    hear_story(cn, co, ppd, &ppd->ruby10, ruby10_story, ptr);
                    break;
                case 11:
                    hear_story(cn, co, ppd, &ppd->ruby11, ruby11_story, ptr);
                    break;
                case 12:
                    hear_story(cn, co, ppd, &ppd->ruby12, ruby12_story, ptr);
                    break;
                case 15:
                    hear_story(cn, co, ppd, &ppd->ruby15, ruby15_story, ptr);
                    break;
                case 16:
                    hear_story(cn, co, ppd, &ppd->ruby16, ruby16_story, ptr);
                    break;
                case 17:
                    hear_story(cn, co, ppd, &ppd->ruby17, ruby17_story, ptr);
                    break;
                }
            }
        }
        ...
        if (msg->type == NT_GIVE) {
            ...
            if ((in = ch[cn].citem)) {
                if ((ch[co].flags & CF_PLAYER) && (ppd = set_data(co, DRD_SIDESTORY_PPD, sizeof(struct sidestory_ppd)))) {
                    switch (areaID) {
                    case 2:
                        ret = give_story(cn, co, ppd, &ppd->ruby2, ruby2_story, in);
                        break;
                    case 5:
                        ret = give_story(cn, co, ppd, &ppd->ruby5, ruby5_story, in);
                        break;
                    case 6:
                        ret = give_story(cn, co, ppd, &ppd->ruby6, ruby6_story, in);
                        break;
                    case 8:
                        ret = give_story(cn, co, ppd, &ppd->ruby8, ruby8_story, in);
                        break;
                    case 10:
                        ret = give_story(cn, co, ppd, &ppd->ruby10, ruby10_story, in);
                        break;
                    case 11:
                        ret = give_story(cn, co, ppd, &ppd->ruby11, ruby11_story, in);
                        break;
                    case 12:
                        ret = give_story(cn, co, ppd, &ppd->ruby12, ruby12_story, in);
                        break;
                    case 15:
                        ret = give_story(cn, co, ppd, &ppd->ruby15, ruby15_story, in);
                        break;
                    case 16:
                        ret = give_story(cn, co, ppd, &ppd->ruby16, ruby16_story, in);
                        break;
                    case 17:
                        ret = give_story(cn, co, ppd, &ppd->ruby17, ruby17_story, in);
                        break;
