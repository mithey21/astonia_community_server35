...
void bank_driver(int cn, int ret, int lastact) {
                ...
                if (ppd && ppd->loaded && (ptr = strcasestr((char *)msg->dat2, "deposit"))) {
                    ...
                    if (val) {
                        if (val > ch[co].gold || val < 0) {
                            ...
                            quiet_say(cn, "Thou hast deposited %s gold coins.", bignumber(val / 100));
                    ...
                    if (val) {
                        if (val > ppd->gold || val < 0) {
                            ...
                            ch[co].gold += val;
                            ch[co].flags |= CF_ITEMS;
                            quiet_say(cn, "Thou hast withdrawn %s gold coins.", bignumber(val / 100));
    ...
    if (ticker > dat->last_talk + TICKS * 60 && !RANDOM(25)) {
        ...
        case 7:
            ...
            break;
        case 8:
            murmur(cn, "All those numbers! I love it.");
            break;
        default:
            break;
        }
