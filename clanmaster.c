...
void clanmaster_driver(int cn, int ret, int lastact) {
    ...
    if (ticker > dat->last_talk + TICKS * 60 && !RANDOM(25)) {
        ...
        case 2:
            ...
            break;
        case 3:
            murmur(cn, "Now where did I put it?");
            break;
        case 4:
            murmur(cn, "Oh my, life is hard but unfair.");
            break;
        case 5:
            murmur(cn, "Beware of the fire snails!");
            break;
        case 6:
            murmur(cn, "I love the clicking of coins.");
            break;
        case 7:
            murmur(cn, "Gold and Silver, Silver and Gold.");
            break;
        default:
            break;
        }
...
void clanclerk_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            switch (res) {
            case 2:
                say(cn, "Our clan has %d jewels.", cnt_jewels(cnr));
                break;
            }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_CLANMASTER:
        clanmaster_driver(cn, ret, lastact);
        return 1;
    case CDR_CLANCLERK:
        clanclerk_driver(cn, ret, lastact);
        return 1;
    case CDR_CLANVAULT:
        clanvault_driver(cn, ret, lastact);
        return 1;
...
int it_driver(int nr, int in, int cn) {
    switch (nr) {
    case IDR_CLANSPAWN:
        clanspawn_driver(in, cn);
        return 1;
    case IDR_CLANJEWEL:
        clanjewel_driver(in, cn);
        return 1;
    case IDR_CLANSPAWNEXIT:
        clanspawn_exit(in, cn);
        return 1;
    case IDR_CLANDEFENSE:
        clandefense_driver(in, cn);
        return 1;
...
int ch_died_driver(int nr, int cn, int co) {
    switch (nr) {
    case CDR_CLANMASTER:
        clanmaster_dead(cn, co);
        return 1;
    case CDR_CLANCLERK:
        clanmaster_dead(cn, co);
        return 1;
    case CDR_CLANVAULT:
        clanvault_dead(cn, co);
        return 1;
