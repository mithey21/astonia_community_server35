...
static int fight_driver_attack_enemy(int cn, int co, int nomove) {
    ...
    if ((!nomove || cdist == 2) && !((int)ch[co].level - (int)ch[cn].level > 13 && (int)ch[co].level - (int)ch[cn].level > (int)ch[cn].level / 4 && (areaID == 4 || areaID == 7 || areaID == 9) && !(ch[cn].flags & CF_PLAYER))) {
    ...
    for (n = 0; n < maxtask; n++) {
        ...
        if (task[n].task == attackback && (n == maxtask - 1 || task[n + 1].task != attack)) ret = 0;
            ...
            case bless:
                ...
                break;
            case fireball:
                ret = fireball_driver(cn, co, ch[co].serial);
                break;
            case ball:
                ret = do_ball(cn, ch[co].x - 1 + RANDOM(3), ch[co].y - 1 + RANDOM(3));
                break;
            case flash:
                ret = do_flash(cn);
                break;
            ...
            case attack:
                ...
                break;
            case moveright:
                ret = do_walk(cn, DX_RIGHT);
                break;
            case moveleft:
                ret = do_walk(cn, DX_LEFT);
                break;
            case moveup:
                ret = do_walk(cn, DX_UP);
                break;
            case movedown:
                ret = do_walk(cn, DX_DOWN);
                break;
            case regenerate:
                ret = do_idle(cn, TICKS / 2);
                break;
            case distance3:
                ret = distance_driver(cn, co, 3);
                if (!ret && error == ERR_ALREADY_THERE) ret = do_idle(cn, TICKS / 4);
                break;
            case distance7:
                ret = distance_driver(cn, co, 7);
                break;
            case earthrain:
                ret = earthrain_driver(cn, co, ch[cn].value[1][V_DEMON]);
                break;
            case earthmud:
                ret = earthmud_driver(cn, co, ch[cn].value[1][V_DEMON]);
                break;
            case heal:
                ret = do_heal(cn, cn);
                break;
            case ms:
                ret = do_magicshield(cn);
                break;
            case attackback:
                ret = attack_back_driver(cn, co);
                break;
            case flee:
                if (cdist < 12) ret = distance_driver(cn, co, 12);
                else {
                    ret = 0;
                    error = ERR_ALREADY_THERE;
                }
                ...
                break;
            case firering:
                ret = do_fireball(cn, ch[cn].x, ch[cn].y);
                break;
...
int add_bonus_spell(int cn, int driver, int strength, int duration) {
    ...
    case IDR_WEAPON:
        ...
        break;
    case IDR_MANA:
        in = create_item("mana_spell");
        break;
    case IDR_HP:
        in = create_item("hp_spell");
        break;
