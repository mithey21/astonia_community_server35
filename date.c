...
void tick_date(void) {
    ...
    // moonlight 0...255, moonsize 0...14
    moonlight = moonlight * moonsize / 223;
...
void showtime(int cn) {
    ...
    nr = (realtime / (BIGSPAWNFREQ)) * BIGSPAWNFREQ + BIGSPAWNFREQ - realtime;
    idx = (realtime / BIGSPAWNFREQ) % 3;
    if (idx == 0) {
        off1 = nr;
        off2 = nr + 1 * BIGSPAWNFREQ;
        off3 = nr + 2 * BIGSPAWNFREQ;
    } else if (idx == 1) {
        off1 = nr + 2 * BIGSPAWNFREQ;
        off2 = nr;
        off3 = nr + 1 * BIGSPAWNFREQ;
    } else {
        off1 = nr + 1 * BIGSPAWNFREQ;
        off2 = nr + 2 * BIGSPAWNFREQ;
        off3 = nr;
    }

    log_char(cn, LOG_SYSTEM, 0, "Next spawn for levels 5 to 38 in: %d:%02d:%02d", off1 / 60 / 60, (off1 / 60) % 60, off1 % 60);
    log_char(cn, LOG_SYSTEM, 0, "Next spawn for levels 40 to 60 in: %d:%02d:%02d", off2 / 60 / 60, (off2 / 60) % 60, off2 % 60);
    log_char(cn, LOG_SYSTEM, 0, "Next spawn for levels 64 to 200 in: %d:%02d:%02d", off3 / 60 / 60, (off3 / 60) % 60, off3 % 60);
}
