...
void update_soldier(int co, int n, struct farmy_ppd *ppd) {
    ...
    ppd->soldier[n].base = 48 + ppd->soldier[n].rank * 4;
...
void fdemon_army(int cn, int ret, int lastact) {
    ...
    if (dat->leader_cn) {
        ...
        switch (dat->mission) {
        case MIS_FOLLOW:
            if (army_follow_driver(cn, dat, 10)) {
                dat->closeup = 1;
                return;
            } else break;
        case MIS_BACK:
            if (army_back_driver(cn, dat)) {
                dat->emote.boredom -= 10;
                return;
            } else break;
        case MIS_RETREAT:
            if (army_follow_driver(cn, dat, 3)) {
                return;
            } else break;
        case MIS_BEHIND:
            if (army_behind_driver(cn, dat)) {
                dat->emote.boredom -= 250;
                return;
            } else break;
        case MIS_FRONT:
            if (army_front_driver(cn, dat, 10)) {
                return;
            } else break;
        }
    }
        ...
        if (msg->type == NT_CHAR) {
            ...
            if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) && ch[cn].driver != CDR_LOSTCON &&
                ...
                case 4:
                    ...
                    break;
                case 5:
                    break; // waiting for mission solve
                case 6:
                    platoon_exp(cn, co, 1000, 2, ppd);
                    ppd->boss_stage++;
                    ppd->boss_timer = realtime;
                    break;
                case 7:
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (res == 8 && (ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) &&
                ...
                case 27:
                    break; // exp give-out
                case 28:
                    break; // just used to reset the counter
                case 29:
                case 30:
                    ppd->boss_stage = 29;
                    ppd->boss_timer = 0;
            ...
            if (strcasestr(ptr, "take") && char_dist(cn, co) < 12 && char_see_char(cn, co)) {
                if (get_army_rank_int(co) < 3) {
                    say(cn, "Your rank is too low, %s.", ch[co].name);
                } else {
                    drop_soldiers(co);
                    take_soldiers(co);
                }
            } else if (strcasestr(ptr, "drop") && char_dist(cn, co) < 12 && char_see_char(cn, co)) {
                drop_soldiers(co);
            }
        }
...
void fdemon_loader(int in, int cn) {
    ...
    if (cn) { // player using item
        ...
        if (it[in].drdata[6] == 1 && (ppd = set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 1 && ppd->boss_stage <= 5) {
    ...
    if (cn) {
        ...
        if (pwr) {
            ...
            if (find_target(in, it[in].x, it[in].y, dx, dy, &shot)) {
                ...
                if (*(unsigned short *)(it[in2].drdata + 1) > lpwr) {
                    *(unsigned short *)(it[in2].drdata + 1) -= lpwr;
                    lpwr = 0;
                } else {
                    lpwr -= *(unsigned short *)(it[in2].drdata + 1);
                    *(unsigned short *)(it[in2].drdata + 1) = 0;
                }
                if (*(unsigned short *)(it[in3].drdata + 1) > lpwr) {
                    *(unsigned short *)(it[in3].drdata + 1) -= lpwr;
                    lpwr = 0;
                } else {
                    lpwr -= *(unsigned short *)(it[in3].drdata + 1);
                    *(unsigned short *)(it[in3].drdata + 1) = 0;
                }
                if (*(unsigned short *)(it[in4].drdata + 1) > lpwr) {
                    *(unsigned short *)(it[in4].drdata + 1) -= lpwr;
                    lpwr = 0;
                } else {
                    lpwr -= *(unsigned short *)(it[in4].drdata + 1);
                    *(unsigned short *)(it[in4].drdata + 1) = 0;
                }
            }
            if (!(it[in].sprite & 1)) {
                it[in].sprite |= 1;
...
void fdemon_gate(int in, int cn) {
    ...
    call_item(it[in].driver, in, 0, ticker + rate * TICKS * 2);
}
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_FDEMON_ARMY:
        fdemon_army(cn, ret, lastact);
        return 1;
    case CDR_FDEMON_BOSS:
        fdemon_boss(cn, ret, lastact);
        return 1;
    case CDR_FDEMON_DEMON:
        fdemon_demon(cn, ret, lastact);
        return 1;
...
int it_driver(int nr, int in, int cn) {
    switch (nr) {
    case IDR_FDEMONLIGHT:
        fdemon_light(in, cn);
        return 1;
    case IDR_FDEMONLOADER:
        fdemon_loader(in, cn);
        return 1;
    case IDR_FDEMONCANNON:
        fdemon_cannon(in, cn);
        return 1;
    case IDR_FDEMONGATE:
        fdemon_gate(in, cn);
        return 1;
    case IDR_FDEMONWAYPOINT:
        fdemon_waypoint(in, cn);
        return 1;
    case IDR_FDEMONFARM:
        fdemon_farm(in, cn);
        return 1;
    case IDR_FDEMONBLOOD:
        fdemon_blood(in, cn);
        return 1;
    case IDR_FDEMONLAVA:
        fdemon_lava(in, cn);
