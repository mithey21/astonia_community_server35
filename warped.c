...
int warpkeydoor_driver(int in, int cn) {
    ...
    if (teleport_char_driver(cn, it[in].x + dx, it[in].y + dy)) {
        ...
        cnt = *(unsigned int *)(it[in2].drdata + 0) = *(unsigned int *)(it[in2].drdata + 0) - 1;
        if (cnt > 1) sprintf(it[in2].description, "%s keys from Rodney's Warped World.", bignumber(*(unsigned int *)(it[in2].drdata + 0)));
        else if (cnt > 0) sprintf(it[in2].description, "A key from Rodney's Warped World.");
        else {
            remove_item_char(in2);
            destroy_item(in2);
        }
...
int give_key(int cn, int cnt) {
    ...
    for (n = 30; n < INVENTORYSIZE; n++) {
        if ((in = ch[cn].item[n]) && it[in].driver == IDR_RWWKEY) {
            *(unsigned int *)(it[in].drdata + 0) += cnt;
            sprintf(it[in].description, "%s keys from Rodney's Warped World.", bignumber(*(unsigned int *)(it[in].drdata + 0)));
            return 1;
        }
    }
    in = create_item("warped_door_key");
    *(unsigned int *)(it[in].drdata + 0) = cnt;
            ...
            if ((in = ch[cn].citem)) {
                ...
                if (it[in].ID == IID_ALCHEMY_INGREDIENT) {
                    if (type == 23) {
                        ...
                        } else say(cn, "Free up some space in your inventory first.");
                    }
                    if (type == 21) {
                        if (give_key(co, 2)) {
                            flag = 1;
...
void collect_keys(int in, int cn) {
    int in2, base;
    ...
    if (!(in2 = ch[cn].citem)) {
        ...
        if (a < 2) {
            log_char(cn, LOG_SYSTEM, 0, "You cannot split one key.");
            return;
        }
        base = it[in].value / a;
        ...
        *(unsigned int *)(it[in].drdata + 0) = a;
        *(unsigned int *)(it[in2].drdata + 0) = b;
        ...
        if (ch[cn].flags & CF_PLAYER) dlog(cn, in2, "took from splitting keys (%s %s)", bignumber(a), bignumber(b));
        ch[cn].citem = in2;
        it[in2].carried = cn;
        ch[cn].flags |= CF_ITEMS;

        log_char(cn, LOG_SYSTEM, 0, "Split into %s keys and %s keys.", bignumber(a), bignumber(b));

        return;
    }
    if (it[in2].driver != it[in].driver) {
        log_char(cn, LOG_SYSTEM, 0, "You cannot mix those.");
        return;
    }

    *(unsigned int *)(it[in].drdata + 0) += *(unsigned int *)(it[in2].drdata + 0);
    it[in].value += it[in2].value;
    sprintf(it[in].description, "%s keys from Rodney's Warped World.", bignumber(*(unsigned int *)(it[in].drdata + 0)));
    log_char(cn, LOG_SYSTEM, 0, "%s keys.", bignumber(*(unsigned int *)(it[in].drdata + 0)));

    if (ch[cn].flags & CF_PLAYER) dlog(cn, in2, "dropped by combining keys");
    destroy_item(in2);
    ch[cn].citem = 0;
    ch[cn].flags |= CF_ITEMS;
}
