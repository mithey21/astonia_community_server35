    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 3:
                switch (dat->nr) {
                case P_SMITH:
                    say(cn, "A smith knows how to repair magic items using silver and gold and uses less raw materials.");
                    break;
                case P_ALCHEMIST:
                ...
                case P_MINER:
                    ...
                    break;
                case P_ENHANCER:
                    say(cn, "The art of the Enhancer allows you to utilize the full power dormant in magic stones. He will create stronger orbs faster than others.");
                    break;
                case P_TRADER:
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_PROFESSOR:
        professor_driver(cn, ret, lastact);
        return 1;
    default:
        return 0;
    }
