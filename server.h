...
struct item {
    ...
    int dummy[4]; // reserve for future extensions
...
#define CF_TESTER (1ull << 6) // character is flagged as tester (various meanings)
#define CF_UPDATE (1ull << 7) // client side update needed (values)
#define CF_DEAD (1ull << 8) // character is dying (death animation is running, doesnt take long)
#define CF_ITEMS (1ull << 9) // client side update needed (items)
#define CF_RESPAWN (1ull << 10) // NPC-flag: respawn?
#define CF_MALE (1ull << 11) // is male
#define CF_FEMALE (1ull << 12) // is female (if neither is set, its neuter)
#define CF_WARRIOR (1ull << 13) // is warrior - or seyan, if warrior and mage are set
#define CF_MAGE (1ull << 14) // is mage - or seyan, if warrior and mage are set
#define CF_ARCH (1ull << 15) // is arch-XXX
#define CF_NOATTACK (1ull << 16) // players cannot select him as attack target
#define CF_HASNAME (1ull << 17) // NPC has a proper name (ie Shiva), not a group name (Orc)
#define CF_QUESTITEM (1ull << 18) // this character can receive quest items through give
#define CF_INFRARED (1ull << 19) // this character has infrared vision (NPC version)
#define CF_ITEMDEATH (1ull << 20) // character turns into an item on death
#define CF_NODEATH (1ull << 21) // character does not actually die (death is handle by driver)
#define CF_NOBODY (1ull << 22) // character does not leave a body behind
#define CF_EDEMON (1ull << 23) // earth-demon capabilities
#define CF_FDEMON (1ull << 24) // fire-demon capabilities
...
#define CF_UNDEAD (1ull << 32) // undead being (for special weapons)
#define CF_HARDKILL (1ull << 33) // 'hard to kill' - needs special weapon to get killed
#define CF_AREACHANGE (1ull << 34) // in-game area change in progress
#define CF_NOTELL (1ull << 35) // does not wish to receive tells
#define CF_INFRAVISION (1ull << 36) // this character has infrared vision (player version)
#define CF_NOMAGIC (1ull << 37) // character is on no-magic tile and no-magic is active
#define CF_NONOMAGIC (1ull << 38) // character is immune to no-magic
#define CF_OXYGEN (1ull << 39) // character is immune to underwater
#define CF_NOPLRATT (1ull << 40) // may not attack/be attack (by) player(s)
#define CF_ALLOWSWAP (1ull << 41) // may be swapped (in spite of being an NPC)
#define CF_NONOTIFY (1ull << 42) // this char does not send out NT_CHAR messages
#define CF_SMALLUPDATE (1ull << 43) // needs set_sector at next act()
#define CF_NOWHO (1ull << 44) // invisible to /who
...
struct character {
    ...
    int mana;
    int lifeshield;
    int regen_ticker; // how long since the last time regeneration was computed?
    ...
    int clan_join_date; // realtime the character joined the clan
    int clan_leave_date; // realtime the character left his last clan

    // staff stuff
    char staff_code[4]; // staffer code for easy recognition

    // statistics
    int login_time; // realtime
    int logout_time; // realtime
    ...
    int dummy[10]; // reserve for extensions
};
