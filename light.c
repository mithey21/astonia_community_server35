...
void compute_shadow(int xc, int yc) {
    ...
    for (y = ys; y < ye; y++) {
        ...
        for (x = xs; x < xe; x++, m++) {
            ...
            if (map[m].fsprite >= 21410 && map[m].fsprite <= 21427) {
                shadow += 31 - (abs(xc - x) + abs(yc - y)) * 5;
                dorand = 1;
            }
            if (map[m].fsprite >= 16000 && map[m].fsprite <= 16007) {
                shadow += 31 - (abs(xc - x) + abs(yc - y)) * 5;
                dorand = 1;
            }
        }
    ...
    for (x = xc - 1, m = yc * MAXMAP + xc - 1; x >= xs; x--, m--) {
        if ((map[m].flags & (MF_SIGHTBLOCK | MF_TSIGHTBLOCK))) {
            shadow += 47 - abs(xc - x) * 10;
            break;
        }
    }
