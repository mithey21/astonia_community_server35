...
void showclan(int cn) {
    ...
    if ((cnr = get_char_clan(cn))) {
        ...
        if (rank < 0 || rank > 4) {
            elog("showclan(): got illegal clan rank (%s,%d,%d)", ch[cn].name, cn, rank);
            rank = 0;
        }
...
void show_clan_relation(int cn, int cnr) {
    ...
    for (n = 1; n < MAXCLAN; n++) {
        ...
        if (crel < 1 || crel > 5) {
            elog("show_clan_relation(): got illegal current_relation %d in clan %d for clan %d", crel, cnr, n);
            crel = 0;
        }
        if (wrel < 1 || wrel > 5) {
...
void tick_clan(void) {
    ...
    case 0:
        ...
        break;
    case 1:
        if (areaID != 13) { // only area 13 updates clan info
    ...
    case 2:
        if (!update_relations()) update_state++;
        break;
    case 3:
        if (!update_treasure()) update_state++;
        break;
    case 4:
        if (clan[cnr].name[0]) schedule_clan_membercount(cnr);
        cnr++;
        if (cnr >= MAXCLAN) cnr = 1;
        update_state++;
        break;
    case 5:
        if (ticker > update_time + TICKS * 10) {
            update_state++;
            update_time = ticker;
        }
        break;
    case 6:
        ...
        break;
    default:
        update_state = 0; // sanity check, should never happen
        break;
    }
}
...
int set_clan_website(int cnr, char *site, int cn) {
    strncpy(clan[cnr].website, site, 79);
    clan[cnr].website[79] = 0;
...
int set_clan_message(int cnr, char *site, int cn) {
    strncpy(clan[cnr].message, site, 79);
    clan[cnr].message[79] = 0;
...
static int update_treasure(void) {
    ...
    for (cnr = 1; cnr < MAXCLAN; cnr++) {
        ...
        if (clan[cnr].treasure.debt >= 1000 && clan[cnr].treasure.jewels > 0) {
            ...
            if (n > clan[cnr].treasure.jewels) {
                n = clan[cnr].treasure.jewels;
                clan[cnr].treasure.jewels = 0;
