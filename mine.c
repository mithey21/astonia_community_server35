...
void minedoor(int in, int cn) {
    ...
    case DX_UP:
        ret = teleport_char_driver(cn, it[in2].x, it[in2].y - 1);
        break;
    case DX_DOWN:
        ret = teleport_char_driver(cn, it[in2].x, it[in2].y + 1);
        break;
    case DX_RIGHT:
        ret = teleport_char_driver(cn, it[in2].x - 1, it[in2].y);
        break;
    case DX_LEFT:
        ret = teleport_char_driver(cn, it[in2].x + 1, it[in2].y);
        break;
    ...
    for (n = 0; n < 9; n++) {
        ...
        for (x = 2 + (n % 3) * 8; x < 9 + (n % 3) * 8; x++) {
            for (y = 231 + (n / 3) * 8; y < 238 + (n / 3) * 8; y++) {
                if ((in2 = map[x + y * MAXMAP].it) && (it[in2].flags & IF_TAKE)) {
                    flag = 1;
                    break;
                }
                if (map[x + y * MAXMAP].ch) {
                    flag = 1;
                    break;
                }
            }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_GOLEMKEYHOLDER:
        keyhold_fight_driver(cn, ret, lastact);
        return 1;
    default:
        return 0;
    }
...
int it_driver(int nr, int in, int cn) {
    ...
    case IDR_MINEDOOR:
        minedoor(in, cn);
        return 1;
    case IDR_MINEKEYDOOR:
        keyholder_door(in, cn);
        return 1;
    case IDR_MINEGATEWAY:
        minegateway(in, cn);
        return 1;
