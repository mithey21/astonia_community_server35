...
void simple_baddy_driver_parse(int cn, struct simple_baddy_driver_data *dat) {
    ...
    for (ptr = nextnv(ch[cn].arg, name, value); ptr; ptr = nextnv(ptr, name, value)) {
        ...
        else if (!strcmp(name, "healer")) dat->healer = atoi(value);
...
void simple_baddy_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        case NT_SEEHIT:
            if (!dat->healer) break;
            if (!ch[cn].value[1][V_HEAL]) break;

            co = msg->dat2;
            if (cn == co) break;
            if (ch[co].group != ch[cn].group) break;
            if (ch[cn].hp < ch[cn].value[0][V_HP] * (POWERSCALE / 2 + POWERSCALE / 4)) break;
            if (ch[co].hp > ch[co].value[0][V_HP] * POWERSCALE / 2) break;
            if (ch[cn].mana < ch[cn].value[0][V_MANA] * POWERSCALE - POWERSCALE) break;
            if (char_dist(cn, co) > 12) break;
    ...
    if (heal_tgt && do_heal(cn, heal_tgt)) return;
