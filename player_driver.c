...
static int check_high_prio_task(int cn, int ac, int ac1, int ac2) {
    switch (ac) {
    case PAC_BLESS:
        if (do_bless(cn, ac1)) return 1;
        else return error_state_mana();
    case PAC_HEAL:
        if (do_heal(cn, ac1)) return 1;
        else return error_state();
    case PAC_MAGICSHIELD:
        if (do_magicshield(cn)) return 1;
        else return error_state();
    }
    return 0;
}
...
static int check_med_prio_task(int cn, int ac, int ac1, int ac2) {
    switch (ac) {
    case PAC_FREEZE:
        if (do_freeze(cn)) return 1;
        else return error_state();
    case PAC_FLASH:
        if (do_flash(cn)) return 1;
        else return error_state();
    case PAC_WARCRY:
        if (do_warcry(cn)) return 1;
        else return error_state();
    }
    return 0;
}
...
static int check_low_prio_task(int cn, int ac, int ac1, int ac2) {
    switch (ac) {
    case PAC_FIREBALL:
        if (do_fireball(cn, ac1, ac2)) return 1;
        else return error_state();
    case PAC_FIREBALL2:
        if (fireball_driver(cn, ac1, ac2)) return 1;
        else return error_state();
    case PAC_BALL:
        if (do_ball(cn, ac1, ac2)) return 1;
        else return error_state();
    case PAC_BALL2:
        if (ball_driver(cn, ac1, ac2)) return 1;
        else return error_state();
    }
    return 0;
}
...
void player_driver(int cn, int ret, int last_action) {
    ...
    case PAC_USE:
        if (player_did_use(nr, cn, last_action)) player[nr]->action = PAC_IDLE;
        if (last_action == AC_USE && ret == 2) player[nr]->action = PAC_IDLE;
        break;
    case PAC_KILL:
        if (ch[player[nr]->act1].serial != player[nr]->act2) player[nr]->action = PAC_IDLE;
        break;
    case PAC_MOVE:
        if (last_action == AC_USE && ret == 2) player[nr]->action = PAC_IDLE;
        break;
    ...
    case PAC_IDLE:
        ...
        return;
    case PAC_MOVE:
        if (!player_move_driver(cn, player[nr]->act1, player[nr]->act2)) {
            player[nr]->action = 0;
            do_idle(cn, 4);
        }
        return;
    case PAC_TAKE:
        if (!take_driver(cn, player[nr]->act1)) {
            player[nr]->action = 0;
            do_idle(cn, 4);
        }
        return;
    case PAC_DROP:
        if (!drop_driver(cn, player[nr]->act1, player[nr]->act2)) {
            player[nr]->action = 0;
            do_idle(cn, 4);
        }
        return;
    case PAC_KILL:
        if (!plr_attack_driver(cn, player[nr]->act1, player[nr]->isautoaction)) {
            player[nr]->action = 0;
            do_idle(cn, 4);
        }
        return;
    case PAC_USE:
        if (!player_use_driver(cn, player[nr]->act1)) {
            player[nr]->action = 0;
            do_idle(cn, 4);
        }
        return;
    ...
    case PAC_LOOK_MAP:
        look_map(cn, player[nr]->act1, player[nr]->act2);
        player[nr]->action = 0;
        do_idle(cn, 4);
        return;
    case PAC_GIVE:
        if (!give_driver(cn, player[nr]->act1)) {
            player[nr]->action = 0;
            do_idle(cn, 4);
        }
        return;
    ...
    case PAC_TELEPORT:
        if (!do_use(cn, ch[cn].dir, player[nr]->act1 + 1)) do_idle(cn, 4);
        player[nr]->action = 0;
        return;
