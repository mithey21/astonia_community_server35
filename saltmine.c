...
void create_worker(int follow_cn, struct saltmine_ppd *ppd) {
    ...
    if (!cn) {
        xlog("create_char() failed in %s %d", __FILE__, __LINE__);
        return;
    }
    ...
    // drop him to map
    if (!drop_char(cn, 179, 19, 0) && !drop_char(cn, 182, 19, 0)) {
        destroy_char(cn);
...
void monk_gatama_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ticker < talkdat->lasttalk + 5 * TICKS) {
                remove_message(cn, msg);
                continue;
            ...
            // only talk to cv
            if (talkdat->cv_co && talkdat->cv_co != co) {
                remove_message(cn, msg);
                continue;
            }
            ...
            case 0:
                if (ppd->salt) ppd->gatamastate = 50;
                else ppd->gatamastate = 10;
                break;
            ...
            // Begin (check)
            case 30:
                if (monksout(co, ppd)) ppd->gatamastate = 40;
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (strcasestr(str, "REPEAT")) {
                ...
                ppd->gatamastate = 0;
            } else if (strcasestr(str, "DETAILS")) {
                ppd->gatamastate = 20;
            } else if (strcasestr(str, "BEGIN")) {
                ppd->gatamastate = 30;
            }
        }
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (co == workerdat->follow_cn) {
                if (strcasestr(str, "COME")) {
                    workerdat->waituntil = 0;
                } else if (strcasestr(str, "WAIT")) {
                    workerdat->waituntil = ticker + 20 * TICKS;
                } else if (strcasestr(str, "SALT")) {
                    log_area(ch[cn].x, ch[cn].y, LOG_SYSTEM, 0, 10, "%s %s his head.", ch[cn].name, workerdat->hassalt ? "nods" : "shakes");
                }
            }
        }
...
int ch_died_driver(int nr, int cn, int co) {
    switch (nr) {
    case CDR_MONK_GATAMA:
        return 1;
    case CDR_MONK_GOVIDA:
        return 1;
    case CDR_MONK_WORKER:
        monk_worker_died_driver(cn, co);
        return 1;
    default:
        return 0;
    }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
    switch (nr) {
    case CDR_MONK_GATAMA:
        monk_gatama_driver(cn, ret, lastact);
        return 1;
    case CDR_MONK_GOVIDA:
        monk_govida_driver(cn, ret, lastact);
        return 1;
    case CDR_MONK_WORKER:
        monk_worker_driver(cn, ret, lastact);
        return 1;
    default:
        return 0;
    }
