/*
 *
 * $Id: base.c,v 1.32 2008/03/29 15:44:10 devel Exp devel $
 *
 * $Log: base.c,v $
 * Revision 1.32  2008/03/29 15:44:10  devel
 * macro less predictable
 *
 * Revision 1.31  2008/02/04 10:12:12  devel
 * added sign driver
 *
 * Revision 1.30  2007/12/03 10:05:02  devel
 * trader token fix
 *
 * Revision 1.29  2007/11/04 12:59:10  devel
 * clan enhancer bonus
 *
 * Revision 1.28  2007/11/04 12:14:06  devel
 * save character after doing a trade
 *
 * Revision 1.27  2007/10/07 11:18:14  devel
 * added min level to randchests
 *
 * Revision 1.26  2007/09/21 10:57:34  devel
 * lf/fire reset shrine
 * small fix to orb usage
 *
 * Revision 1.25  2007/08/15 09:12:43  devel
 * V_COLD and orbs
 *
 * Revision 1.24  2007/08/09 11:11:00  devel
 * beta item creator
 *
 * Revision 1.23  2007/07/24 18:53:49  devel
 * trader can trade tokens
 *
 * Revision 1.22  2007/07/13 15:47:16  devel
 * clog -> charlog
 *
 * Revision 1.21  2007/07/11 12:37:27  devel
 * can no longer take pots while dying
 * removed health increase from apples etc
 *
 * Revision 1.20  2007/07/04 09:19:39  devel
 * added enhancer profession
 *
 * Revision 1.19  2007/07/02 08:58:45  devel
 * added logging to healing pots
 *
 * Revision 1.18  2007/07/02 08:47:05  devel
 * healing potions and spells now do their work over time instead of instantly
 *
 * Revision 1.17  2007/07/01 13:26:33  devel
 * reduced buried treasure in bran forest
 *
 * Revision 1.16  2007/06/22 13:01:14  devel
 * made macro daemon harder to fool
 *
 * Revision 1.15  2007/06/13 15:22:34  devel
 * macro daemon uses new punishment system
 * macro daemon no longer honors LOSTCON
 * macro daemon comes more often
 *
 * Revision 1.14  2007/06/11 10:03:57  devel
 * removed beyond potion driver
 *
 * Revision 1.13  2007/06/08 12:07:33  devel
 * made orbs combine-able
 * changed orb-spawner into exploration point
 *
 * Revision 1.12  2007/06/07 15:01:11  devel
 * changed item base costs from 1000 to 250
 *
 * Revision 1.11  2007/05/02 13:05:11  devel
 * made recalls not work in area 13
 *
 * Revision 1.10  2007/05/02 12:32:29  devel
 * no potions in clan halls
 *
 * Revision 1.9  2007/03/08 12:07:01  devel
 * fixed bug in spawn time calculations
 *
...
void macro_driver(int cn, int ret, int lastact) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (co == dat->victim && ch[co].ID == dat->v_ID) {
                ...
                if (val == dat->val1 + dat->val2) {
                    ...
                    dat->start -= TICKS * 30;
                } else if (strstr(text, "repeat")) {
                    say(cn, "What is %d plus %d, %s?", dat->val1, dat->val2, ch[co].name);
                }
    ...
    if (dat->state == 1) { // teleport to victim
        co = dat->victim;
        if ((ch[co].flags & CF_PLAYER) && ch[co].ID == dat->v_ID && macro_teleport_char_driver(cn, ch[co].x, ch[co].y)) {
    ...
    if (dat->state == 3) {
        co = dat->victim;
        if (ch[co].ID != dat->v_ID || !(ch[co].flags & CF_PLAYER)) {
            dat->state = 0;
            dat->victim++;
            do_idle(cn, TICKS);
            return;
        }
        if (!char_see_char(cn, co) || char_dist(cn, co) > 10) {
            teleport_char_driver(cn, ch[co].x, ch[co].y);
        }
    ...
    if (dat->state == 4) {
        ...
        sprintf(buf, "%s punished for not answering within 5 minutes by Macro Daemon", ch[co].name);
        punish(0, ch[co].ID, buf);
        dat->state = 0;
        dat->victim++;
...
void potion_driver(int in, int cn) {
    ...
    if (areaID == 33 || areaID == 13) { // long tunnel: no potions!
        log_char(cn, LOG_SYSTEM, 0, "You sense that the potion would not work.");
        return;
    }
    if (ch[cn].action == AC_DIE) {
        log_char(cn, LOG_SYSTEM, 0, "Too late. You're dying already.");
        return;
    }
    ...
    if (ch[cn].flags & CF_PLAYER) dlog(cn, in, "Used a potion");
...
void chest_driver(int in, int cn) {
    ...
    if (it[in].min_level > ch[cn].level) {
        log_char(cn, LOG_SYSTEM, 0, "You're not strong enough to use this (minimum level required is %d).", it[in].min_level);
        return;
    }
...
void recall_driver(int in, int cn) {
    ...
    if (areaID == 13) {
        log_char(cn, LOG_SYSTEM, 0, "This scroll does not work here.");
        return;
    }
...
void teleport_door_driver(int in, int cn) {
    ...
    if (it[in].drdata[1] && (map[x + y * MAXMAP].flags & MF_CLAN)) {
        int t;
        t = realtime - ((realtime / (BIGSPAWNFREQ)) * BIGSPAWNFREQ + BIGSPAWNFREQ);
        if (t < -(BIGSPAWNFREQ / 2)) t += BIGSPAWNFREQ;

        if (t < 0 || t > 65 * 60) {
            log_char(cn, LOG_SYSTEM, 0, "Entering Clan Spawner.");
        } else {
            log_char(cn, LOG_SYSTEM, 0, "Big spawn in progress. You cannot enter for %d minutes.", 65 - t / 60);
            return;
        }
    }
...
void randchest_driver(int in, int cn) {
    int ID, n, old_n = 0, old_val = 0, in2 = 0, amount, minlevel;
    struct randchest_ppd *ppd;

    minlevel = max(0, it[in].drdata[0] / 2 - 10);
    ...
    if (!in2) {
        amount = (RANDOM(it[in].drdata[0]) + 1) * (RANDOM(it[in].drdata[0]) + 1) * 4;
...
void food_driver(int in, int cn) {
    ...
    case 0:
        log_area(ch[cn].x, ch[cn].y, LOG_INFO, cn, 10, "%s eats a %s. You see juice dripping down %s chin.", ch[cn].name, it[in].name, hisname(cn));
        break;
    case 1:
        log_area(ch[cn].x, ch[cn].y, LOG_INFO, cn, 10, "%s eats a bowl of %s. A moment later, flames burst from %s ears.", ch[cn].name, it[in].name, hisname(cn));
        break;
    case 2:
        lollipop(in, cn);
        return;
...
void enchant_item(int in, int cn) {
    int in2, n, v, cost = 0, cost2, cnt = 0;
    double fraction;
    ...
    if (it[in2].ID == IID_ALCHEMY_INGREDIENT && it[in2].drdata[0] >= 0x15 && it[in2].drdata[0] <= 0x18) {
        it[in].quality +=
            it[in2].quality *
            (1.0 + get_support_prof(cn, P_ENHANCER) / 100.0) *
            (1.0 + clan_enhance_bonus(cn) / 100.0);
        it[in].complexity +=
            it[in2].complexity *
            (1.0 + get_support_prof(cn, P_ENHANCER) / 100.0) *
            (1.0 + clan_enhance_bonus(cn) / 100.0);
        ...
        destroy_item(in2);
        ch[cn].flags |= CF_ITEMS;
        log_char(cn, LOG_SYSTEM, 0, "You added the stone to the orb. New Strength: %s, new Complexity: %s.", bignumber(it[in].quality), bignumber(it[in].complexity));
        return;
    }
    if (it[in2].driver == 83) {
        it[in].quality += it[in2].quality;
        it[in].complexity += it[in2].complexity;
        ch[cn].citem = 0;
        it[in].value = it[in].quality * 10;
        destroy_item(in2);
        ch[cn].flags |= CF_ITEMS;
        log_char(cn, LOG_SYSTEM, 0, "You combined the two orbs. New Strength: %d, new Complexity: %d.", it[in].quality, it[in].complexity);
        return;
    }
    ...
    for (n = 0; n < MAXMOD; n++) {
        ...
        case V_COLD:
            break;
        default:
            if ((v = it[in2].mod_value[n]) > 0 && it[in2].mod_index[n] >= 0 && v < 20) {
                cost += v * 250 + 250;
                cnt++;
            }
        }
    ...
    if (cnt >= 3) cost *= 2;
    if (cost <= 0) {
        log_char(cn, LOG_SYSTEM, 0, "You cannot enhance %s using an Orb.", it[in2].name);
        return;
    }
    if (cost > it[in].quality) {
        log_char(cn, LOG_SYSTEM, 0, "The orb is not strong enough to enhance the %s. It has a strength of %d, but would need %d.",
                 it[in2].name, it[in].quality, cost);
        return;
    }
    fraction = (double)cost / it[in].quality;
    cost2 = fraction * it[in].complexity;
    it[in].quality -= cost;
    it[in].complexity -= cost2;
    it[in2].quality += cost;
    it[in2].complexity += cost2;

    if (it[in].quality < 1) it[in].quality = 1;
    if (it[in].complexity < 1) it[in].complexity = 1;

    it[in].value = it[in].quality * 10;
    it[in2].value = it[in2].quality * 10;
...
void explore_driver(int in, int cn) {
    int ID, n, expval, full;
    ...
    for (n = 0; n < MAXEXPLORE; n++) {
        if (ppd->ID[n] == ID) {
            log_char(cn, LOG_SYSTEM, 0, "You've been here before.");
            return;
        }
    }
    for (n = 0; n < MAXEXPLORE; n++) {
    ...
    if (n == MAXEXPLORE) {
        log_char(cn, LOG_SYSTEM, 0, "You have already found %d exploration points; The system cannot handle any more of them. Please go shout at the game admins.", MAXEXPLORE);
        return;
    }

    ppd->ID[n] = ID;

    full = level_value(it[in].drdata[0]) / 10;
    expval = min(full, level_value(ch[cn].level) / 10);

    dlog(cn, 0, "Got %d (of %d) exp from exploration point.", expval, full);
    give_exp(cn, expval);
    log_char(cn, LOG_SYSTEM, 0, "You've been blessed with experience.");
}
    ...
    if (areaID == 29 && (ppd = set_data(cn, DRD_STAFFER_PPD, sizeof(struct staffer_ppd)))) {
        ...
        case 0:
            if (ch[cn].x == 83 && ch[cn].y == 127) {
                in2 = create_money_item(250 * 100 + RANDOM(250 * 100));
                ppd->forestbran_done++;
            }
            break;
        case 1:
...
void special_potion(int in, int cn) {
    ...
    case 3:
    case 4:
        if (type >= 0 && type <= 3) flag = remove_poison(cn, type);
...
void return_items(struct trader_data *dat, int switched) {
    ...
    for (n = 0; n < dat->c1cnt; n++) {
        ...
        if (!c2 || is_gk_room(c2) || !give_char_item(c2, dat->c1itm[n])) {
            if (switched) dlog(0, dat->c1itm[n], "%s: lost item in trade", dat->c2name);
            else dlog(0, dat->c1itm[n], "%s: lost item in trade", dat->c1name);
            destroy_item(dat->c1itm[n]);
        } else dlog(c2, dat->c1itm[n], "was given from trader");
        dat->c1itm[n] = 0;
    }
    dat->c1cnt = 0;
    ...
    if (dat->c1token) {
        if (c2 && (ppd = set_data(c2, DRD_DEPOT_PPD, sizeof(struct depot_ppd))) && ppd->loaded) {
            ...
            if (switched) {
                ...
                add_note(dat->s2ID, NOTE_INFO, "%s received %d token(s) from %s (sID=%d)", dat->c2name, dat->c1token, dat->c1name, dat->s1ID);
                add_note(dat->s1ID, NOTE_NOTE, "%s gave %d payment token(s) to %s", dat->c1name, dat->c1token, dat->c2name);
                add_note(dat->s2ID, NOTE_NOTE, "%s received %d payment token(s) from %s", dat->c2name, dat->c1token, dat->c1name);
            } else {
            ...
            if (switched) elog("%s: lost %d payment tokens in trade", dat->c2name, dat->c1token);
            else elog("%s: lost %d payment tokens in trade", dat->c1name, dat->c1token);
        }
        dat->c1token = 0;
    }
    ...
    for (n = 0; n < dat->c2cnt; n++) {
        ...
        if (!c2 || is_gk_room(c2) || !give_char_item(c2, dat->c2itm[n])) {
            if (switched) dlog(0, dat->c2itm[n], "%s: lost item in trade", dat->c1name);
            else dlog(0, dat->c2itm[n], "%s: lost item in trade", dat->c2name);
            destroy_item(dat->c2itm[n]);
        } else dlog(c2, dat->c2itm[n], "was given from trader");
        dat->c2itm[n] = 0;
    }
    dat->c2cnt = 0;
    ...
    if (dat->c2token) {
        if (c2 && (ppd = set_data(c2, DRD_DEPOT_PPD, sizeof(struct depot_ppd))) && ppd->loaded) {
            ppd->payment_tokens += dat->c2token;

            if (switched) {
                xlog("trader gave %d payment tokens to %s from trade with %s for a total of %d", dat->c2token, dat->c1name, dat->c2name, ppd->payment_tokens);
                add_note(dat->s2ID, NOTE_INFO, "%s gave %d token(s) to %s (sID=%d)", dat->c2name, dat->c2token, dat->c1name, dat->s1ID);
                add_note(dat->s1ID, NOTE_INFO, "%s received %d token(s) from %s (sID=%d)", dat->c1name, dat->c2token, dat->c2name, dat->s2ID);
                add_note(dat->s2ID, NOTE_NOTE, "%s gave %d payment token(s) to %s", dat->c2name, dat->c2token, dat->c1name);
                add_note(dat->s1ID, NOTE_NOTE, "%s received %d payment token(s) from %s", dat->c1name, dat->c2token, dat->c2name);
            } else {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            if (strstr(text, "give token")) {
                ...
                if (!(ppd = set_data(co, DRD_DEPOT_PPD, sizeof(struct depot_ppd))) || !ppd->loaded) {
                    quiet_say(cn, "Sorry, it seems your depot was not loaded.");
                    talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
                    remove_message(cn, msg);
                    continue;
                }
                if (ppd->payment_tokens < 1) {
                    quiet_say(cn, "Sorry, you do not have a payment token.");
                    talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
                    remove_message(cn, msg);
                    continue;
                }
                if (char_dist(cn, co) > 3) {
                    quiet_say(cn, "You have to stand next to me to give me a token (this is to avoid a bug).");
                    talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
                    remove_message(cn, msg);
                    continue;
                }
    ...
    if (use_driver(cn, dat->light[0], 0)) {
        if (!RANDOM(50)) {
            ...
            case 1:
                ...
                break;
            case 2:
                murmur(cn, "Infravision potions. Yes, that's a good way to deal with the dark!");
                break;
            case 3:
...
void shrike_amulet_driver(int in, int cn) {
    ...
    case 5:
        ...
        sprintf(it[in].description, "A light blue crystal on a silver crescent charm.");
        break;
    case 6:
...
void betaitemmaker(int in, int cn) {
    int in2;

    if (!cn) return;

    if (ch[cn].exp_used > ch[cn].exp) {
        log_char(cn, LOG_SYSTEM, 0, "You cannot use this shrine while you have negative experience.");
        return;
    }
    if (ch[cn].flags & CF_TESTER) {
        log_char(cn, LOG_SYSTEM, 0, "You have used this shrine already.");
        return;
    }

    if (!(in2 = ch[cn].citem) || it[in2].value != 7) {
        log_char(cn, LOG_SYSTEM, 0, "This shrine will reset your Fire and Lightning to 1. You can only use this shrine once, so re-raise carefully.");
        log_char(cn, LOG_SYSTEM, 0, "You will not lose any experience by using the shrine. To use it, take 7 silver in your hand (mouse cursor) and use them on the shrine.");
        return;
    }

    if (!ch[cn].value[1][V_FIRE] || !ch[cn].value[1][V_FLASH]) {
