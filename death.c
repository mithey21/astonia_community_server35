...
void check_military_solve(int cn, int co) {
    ...
    if (ppd->took_mission && !ppd->solved_mission) {
        ...
        case 1:
            if ((ch[co].class >= 52 && ch[co].class <= 84) || (ch[co].class >= 107 && ch[co].class <= 170) || (ch[co].class >= 388 && ch[co].class <= 403)) { // its a pent demon
            ...
            break;
        case 2:
            if (ch[co].class >= 85 && ch[co].class <= 100) { // its a sewer ratling
                if (ch[co].level == ppd->mis[nr].opt2) {
                    ppd->mis[nr].opt1--;
                    if (ppd->mis[nr].opt1) {
                        if (ppd->mis[nr].opt1 < 10 ||
...
int drop_grave(int in, int x, int y, int isplayer) {
    ...
    switch (areaID) {
    case 8:
        return drop_item_extended(in, 230, 233, 5);
    default:
        return 0;
    }
}
...
int die_char(int cn, int co, int ispk) {
    ...
    if (ch[cn].flags & CF_NOBODY) {
        ...
        if (in) {
            ...
            switch (sprite) {
            default:
                it[in].sprite = 100000 + sprite * 1000 + (dir - 1) / 2 * 8 + 335;
                break;
            }
    ...
    if (!(ch[cn].flags & (CF_ITEMDEATH | CF_NOBODY)) && in && (ct = create_item_container(in))) {
        ...
        if (ch[cn].flags & CF_PLAYER) offset = 12;
        else offset = 0;
...
int hurt(int cn, int dam, int cc, int armordiv, int armorper, int shieldper) {
    ...
    if (dam < 0) {
        elog("got negative damage %d to %s (%d) from %s (%d). Set to 0.", dam, ch[cn].name, cn, ch[cc].name, cc);
        dam = 0;
    }
