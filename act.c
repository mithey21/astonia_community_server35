...
static int act(int cn) {
    ...
    case AC_DROP:
        return act_drop(cn);
    case AC_ATTACK1:
        return act_attack(cn);
    case AC_ATTACK2:
        return act_attack(cn);
    case AC_ATTACK3:
        return act_attack(cn);
    case AC_FIREBALL1:
        return act_fireball(cn);
    case AC_FIREBALL2:
        return act_ok(cn);
    case AC_MAGICSHIELD:
        return act_magicshield(cn);
    case AC_HEAL_SELF:
        return act_heal(cn, 0);
    case AC_HEAL1:
        return act_heal(cn, 1);
    case AC_HEAL2:
        return act_ok(cn);
    case AC_BLESS_SELF:
        return act_bless(cn, 0);
    case AC_BLESS1:
        return act_bless(cn, 1);
    case AC_BLESS2:
        return act_ok(cn);
    case AC_DIE:
        return act_die(cn);
    ...
    case AC_GIVE:
        return act_give(cn);
    case AC_WARCRY:
        return act_warcry(cn);
    case AC_EARTHRAIN:
        return act_earthrain(cn);
    case AC_EARTHMUD:
        return act_earthmud(cn);
    case AC_FIRERING:
        return act_firering(cn);

    default:
        elog("act(): unknown action %d for character %s (%d)", ch[cn].action, ch[cn].name, cn);
        return 0;
    }
}
...
void tick_char(void) {
    ...
    for (n = getfirst_char(); n; n = next) {
        ...
        if (!ch[n].flags) {
            xlog("tick_char: got unused char %d (%s) from getfirst/getnext (%d)!!", n, ch[n].name, next);
            break;
        }
        ...
        if (ch[n].step && ch[n].flags) {
            ...
            prof = prof_start(19);
            char_driver(ch[n].driver, CDT_DRIVER, n, ret, lastact);
            prof_stop(19, prof);
        }
